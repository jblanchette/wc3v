...
const CampCreditModel = require("./CampCreditModel");
...
// work"; players accrue that work over time as they fight in it. The moment a
// TEAM's cumulative work completes the camp is the `clearedTime` — any
// engagement AFTER it earns ZERO credit (walking through an emptied camp).
// Work pools per team, not globally: two enemy armies meeting at a camp are
// mostly fighting each other, and summing both sides cleared contested camps
// at roughly twice the true rate. Each player is credited, per-contribution and
// time-ordered, for the share of the clear they did BEFORE it was cleared.
// The clearing-work model itself (how fast work accrues, what a camp needs,
// when it is cleared) lives in lib/CampCreditModel.js so the tuning harness
// tools/camp-clear-audit.js grades the exact model that ships. Only the
// per-player CREDIT criteria — who gets named as a clearer — stay here.
const CAMP_CREDIT = {
...
// presence accounting (the work model's own gap rule lives in CampCreditModel)
...
const NeutralGroup = class {
  ...
  _computePlayerCredit () {
    ...
    // ── clearing work + estimated clear time ─────────────────────────────
    // Shared with the tuning harness — see lib/CampCreditModel.js.
    const estimate = CampCreditModel.estimateClear({
      totalLevel: this.totalLevel,
      hasFountain: this.hasFountain,
      perPlayerEvents: all
    });
    const chunks = estimate.chunks;
    const requiredMs = estimate.requiredMs;
    this.clearedTime = estimate.clearedTime;
