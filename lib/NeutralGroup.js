...
//
// if a player is above the contested threshold and no other
// player interacts with the camp we grant them it
//
const EMPTY_CLAIM_THRESHOLD = 12.5 * 1000;
        ...
        if (type == 'exited' || type == 'within') {
          ...
          const isFullClaim = Object.keys(adjustedClaimTimings).find(claimState => {
            ...
            if (claimers[teamId].timeClaimed >= claimTiming) {
              ...
              this.assignClaim(ClaimStates[claimState], gameTime, teamId);
          ...
          if (isFullClaim) {
            ...
            //
            // when we have a full claim, determine if any of the 
            // other team claims for this spot meet the criteria
            // to be considered contested  
            //

            const isContested = otherClaims.find(claim => {
              //
              // must be:
              //  * time claimed above the contested claim timing threshold
              //  * last seen game timer before the current claims last seen timer
              //
              //  this helps filter out cases where a player roams through a camp
              //  well after it's already been taken
              //

              return (claim.timeClaimed >= ClaimTimings.contested)
                && (claim.lastTimer <= currentClaim.lastTimer);
            });
            ...
            //
            // if we are the only claimer and we meet the
            // 'empty claim threshold' then assign us as the winner
            //

            if (!otherClaims.length) {
              this.assignClaim(ClaimStates.claimed, gameTime, teamId);
