...
const SubGroup = class {
  ...
  // Union, not concat. A SELECT action can re-list units that are already in
  // the selection (WC3 re-sends the whole set when a large selection is split
  // across actions), and a plain concat made every one of those a duplicate
  // entry — inflating numberUnits, and handing the same unit repeated orders
  // once selectMode=1 started merging by default.
  mergeGroups (otherGroup) {
    otherGroup.units.forEach(unit => {
      const already = this.units.find(existing =>
        utils.isEqualUnitItemId(existing, unit));
