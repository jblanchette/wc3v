...
function resolveSurround (units, shape, player, walkTest, opts = {}) {
  ...
  if (melee.length) {
    ...
    for (const m of order) {
      ...
      for (const c of candidates) {
        if (c.taken) continue;
        // Ring dominates: fill ring 0 before ring 1. Within a ring, the
        // smallest angular detour from the unit's approach bearing wins.
        const key = c.ring * 100 + Math.abs(normAng(c.angle - m.bearing));
        if (key < bestKey - 1e-9 || (Math.abs(key - bestKey) <= 1e-9 && best && c.index < best.index)) {
          best = c; bestKey = key;
        }
      }
      if (best) {
        best.taken = true;
        slots.set(m.unit, { x: best.x, y: best.y });
      }
    }
  }
  ...
  return { slots, candidates, unassigned };
}
