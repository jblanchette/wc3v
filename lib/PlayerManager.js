...
const PlayerManager = class {
  ...
  makePlayer (id) {
    ...
    console.logger("calling setup init units");
    player.setupInitialUnits();

    console.logger("JDEBUG finished call makePlayer: ", id, playerSlot);
  }

  checkCreatePlayer (actionBlock) {
    console.logger("JDEBUG check create player: ", actionBlock);
    const playerId = actionBlock.playerId;

    if (!this.players[playerId]) {
      this.makePlayer(playerId);
    } else {
      console.logger("JDEBUG already made player: ", playerId);
    }
  }
