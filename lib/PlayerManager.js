...
const PlayerManager = class {
  ...
  _isDuplicateAbilityAction (player, actionName, action, gameTime) {
    ...
    const prev = player._lastAbilityAction;
    player._lastAbilityAction = { sig, gameTime };

    if (!prev) return false;
    if (prev.sig !== sig) return false;
    return (gameTime - prev.gameTime) < ABILITY_DEDUP_WINDOW_MS;
  }
  ...
  _abilityActionSignature (actionName, action) {
    ...
    if (action.targetX !== undefined) parts.push(Math.round(action.targetX));
    if (action.targetY !== undefined) parts.push(Math.round(action.targetY));
    if (action.targetA !== undefined) parts.push(Math.round(action.targetA));
    if (action.targetB !== undefined) parts.push(Math.round(action.targetB));
