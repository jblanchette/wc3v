...
// checksum -> mapDataByFile key, for replays whose map filename is a hash
// rather than a name. OPTIONAL: the table is a local artifact and is absent
// from a clean checkout, in which case this is a no-op and map resolution
// behaves exactly as it always has.
let mapAliases = {};
try {
  mapAliases = require("../helpers/mapAliases.json").aliases || {};
} catch (e) { /* no alias table present — normal resolution only */ }
...
const PlayerManager = class {
  constructor (parseOptions = {}) {
    ...
    this.world = new World(this.eventTimer, parseOptions);
  ...
  setGridData (meta, options = {}) {
    ...
    const mapName = normalizeMapName(meta.map.mapName);

    // Resolution rules live in helpers/mapResolver.js so the browser parser,
    // the summary extractor and the start-position lookup all agree with this
    // one. They used to each carry their own copy of the scan and drifted.
    const mapDataName = resolveMapKey(meta.map.mapName, mapDataByFile);

    // Checksum fallback. NetEase / Reforged-client replays name their maps by
    // MD5 (Maps/dz/versus_reforged/3fb7c0bf....w3x), which matches nothing in
    // mapDataByFile even though the underlying map is a standard ladder map.
    // The replay carries its own mapChecksumSha1, so a checksum→key table
    // recovers those. Consulted ONLY after normal resolution has failed, so it
    // cannot change how an already-working replay resolves.
    let resolvedName = mapDataName;
    if (!mapDataByFile[resolvedName]) {
      const sha1 = meta.map && meta.map.mapChecksumSha1;
      const aliased = sha1 ? mapAliases[String(sha1).toLowerCase()] : null;
      if (aliased && mapDataByFile[aliased]) {
        resolvedName = aliased;
        // Rewrite the map name to the resolved key, keeping the original for
        // provenance. Downstream consumers re-derive the map from this string
        // rather than from mapData — PlayerActions.js start-position lookup is
        // the one that matters — so rewriting here fixes all of them at once
        // instead of teaching each about checksums. Only ever runs on the
        // alias path, i.e. for replays that would otherwise have thrown.
        meta.map.mapNameOriginal = meta.map.mapName;
        meta.map.mapName = aliased;
      }
    }

    const mapData = mapDataByFile[resolvedName];
    if (!mapData) {
      ...
      throw new MissingMapError("missing-map-data", {
        ...
        mapChecksumSha1: (meta.map && meta.map.mapChecksumSha1) || null
      });
    }
