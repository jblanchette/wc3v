...
const PlayerManager = class {
  ...
  groupNeutralUnits () {
    ...
    const treeSize = 312; // approx unit size to always collide with neighbors
    ...
    const groupsWithBounds = groups
      ...
      .map(group => {
        ...
        return {
          bounds,
          group: group.map(item => item.unit) // remove extra properties and just leave the unit
        };
      });

    this.world.setNeutralGroups(groupsWithBounds);
  }
