...
const PlayerActions = class {
	...
	static moveSelectedUnits (
		...
		units.forEach(unit => {
			console.logger("move sel unit.moveTo: ", unit.displayName);
	...
	static checkUnitBackfill (
		...
		player.possibleSelectList.forEach(possibleUnit => {
			if (utils.isEqualUnitItemId(backfillUnit, possibleUnit)) {
				console.logger("found backfill: ", possibleUnit);
		...
		if (existingUnits.length === 1) {
			...
			if (unregisteredUnit.isUnit) {
				...
				if (unregisteredWorker) {
					...
					PlayerActions.checkUnitBackfill(player, unregisteredWorker);
				}
			}
				...
				if (!newlyRegisteredUnits.length) {
					...
					if (switchUnitByItemId) {
						...
						if (!meta.permanent) {
							...
							// we only know the objectId1-2 for sure
							// create a unit without setting itemId1-2
							...
							console.logger("jeff added unit to selection: ", itemId1, itemId2);
