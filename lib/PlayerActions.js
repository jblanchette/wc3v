...
const { getAcquisitionRange, getEffectiveRange } = require("../helpers/effectiveRange");
...
const PlayerActions = class {
  ...
  static moveSelectedUnits (
    ...
    // ---- Combat formation (concave army-vs-army, or siege surround) -------
    //
    // For an attack order — attack/right-click on an enemy unit or building
    // (kind 'attack') or an attack-move to GROUND (kind 'attackground') — we
    // reconstruct where the engine would settle each unit: melee front /
    // ranged back on a concave against a mobile enemy, or perimeter rings
    // around a building footprint when the destination holds only buildings.
    // Computed ONCE over the whole selected army — INCLUDING units the
    // coalescer below will keep on their current path, whose endpoint is
    // refined to their slot rather than the raw click.
    //
    // Still gated on actually finding a nearby enemy: an attack-move across
    // empty ground produces NO slots and falls through to the normal
    // point-move, so strict MoveCommands and empty-map advances are unchanged.
    const combatOrder = (opts.kind === 'attack' || opts.kind === 'attackground');
    let attackSlots = null;   // Map<unit,{x,y}>
    let attackCtx = null;     // { mode, candidates?, shape? } — surround retry context
    if (combatOrder) {
      const form = PlayerActions._resolveCombatFormation(
        player, world, units, targetX, targetY, opts
      );
      if (form) { attackSlots = form.slots; attackCtx = form; }
      ...
      if (orderSel.length && world && typeof world.findEnemyUnitsNear === 'function') {
        // includeBuildings: an attack on an undefended base is still an attack.
        // Without it, no mobile defender within 900u meant NO order was ever
        // recorded during a siege, and the viewer idled every attacker for
        // want of corroboration. The "go mine" false positive stays impossible:
        // gold mines are neutral-owned and rejected upstream of the building
        // filter.
        const enemiesNear = world.findEnemyUnitsNear(
          targetX, targetY, 900, player.id, player.teamId, true
        );
        if (enemiesNear.length) {
          const orderGT = player.eventTimer.timer.gameTime;
          for (const u of orderSel) {
            ...
            u.recordOrder(orderGT, opts.orderKind || opts.kind, {
            ...
            if (lastT == null || (orderGT - lastT) > 1000) u.combatOrderTimes.push(orderGT);
          }
        }
      }
    }
    ...
    const coalesced = new Set();
    {
      units.forEach(unit => {
        ...
        // keep moving; just refine the final point. For a combat order the
        // refined point is the unit's formation SLOT — the raw-click rewrite
        // used to drag every coalesced attacker onto the clicked enemy/building
        // center, silently discarding the slot the formation just computed.
        const dest = (attackSlots && attackSlots.has(unit))
          ? attackSlots.get(unit)
          : { x: targetX, y: targetY };
        if (unit.walkPath && unit.walkPath.length) {
          const last = unit.walkPath[unit.walkPath.length - 1];
          unit.walkPath[unit.walkPath.length - 1] = { x: dest.x, y: dest.y, weight: last.weight };
        } else if (unit.moveInfo) {
          unit.moveInfo.targetX = dest.x;
          unit.moveInfo.targetY = dest.y;
        }
        unit._moveTarget = { x: dest.x, y: dest.y };
        coalesced.add(unit);
      });
    }
    ...
    // ---- slot dispatch (shared by every pathfind group) -------------------
    //
    // A slotted unit paths to its slot with its OWN A*. Failure semantics are
    // the load-bearing part: the old dispatcher silently returned when the
    // pathfinder came back empty and the unit was >16u out — the unit froze
    // where it stood and nothing ever re-evaluated — and otherwise issued an
    // unconditional straight hop that walked units through stamped building
    // footprints. Now nothing freezes: a unit either reaches a slot it can
    // fight from, retries the next free perimeter slot (budgeted, via the
    // arrival callback), or falls back to a plain move at the raw target,
    // whose endpoint relocation lands on the nearest open cell at the
    // footprint edge.
    const formStats = () => player._formApply ||
      (player._formApply = { applied: 0, emptyPath: 0, retried: 0, fellBack: 0, heldAtSlot: 0 });

    const fallbackPlainMove = (unit) => {
      if (config.debugFormation) formStats().fellBack++;
      const wi = world.pathFinder.findPath(unit.currentX, unit.currentY, targetX, targetY);
      if (wi.walkPath.length) unit.moveTo(world, wi, targetX, targetY);
    };

    // Within weapon range of the siege footprint's EDGE (plus collision size
    // and one cell of slack)?
    const inAttackReach = (unit, shape) => {
      if (!shape) return true;
      const range = Math.max(64, getEffectiveRange(unit.itemId, player) || 0);
      const slack = ((unit.balanceInfo && unit.balanceInfo.collisionSize) || 16) + 32;
      if (shape.kind === 'circle') {
        return Math.hypot(unit.currentX - shape.x, unit.currentY - shape.y)
          <= shape.radius + range + slack;
      }
      const ex = Math.max(0, Math.abs(unit.currentX - shape.x) - shape.halfW);
      const ey = Math.max(0, Math.abs(unit.currentY - shape.y) - shape.halfH);
      return Math.hypot(ex, ey) <= range + slack;
    };

    const SLOT_RETRY_BUDGET = 3;

    const dispatchToSlot = (unit, slot, budget) => {
      const wi = world.pathFinder.findPath(unit.currentX, unit.currentY, slot.x, slot.y);
      if (wi.walkPath.length) {
        unit.moveTo(world, wi, slot.x, slot.y, (finished) => {
          // finished=false means a newer order redirected the unit mid-walk —
          // that order owns it now. Retries only ever fire on real arrival,
          // inside the deterministic EventTimer completion, so re-dispatch
          // order is a pure function of the replay clock.
          if (finished !== true) return;
          if (!attackCtx || attackCtx.mode !== 'surround') return;
          if (inAttackReach(unit, attackCtx.shape)) return;
          if (budget <= 0) return fallbackPlainMove(unit);
          if (config.debugFormation) formStats().retried++;
          const next = CombatFormation.claimCandidate(
            attackCtx.candidates, unit.currentX, unit.currentY, attackCtx.shape);
          if (next) dispatchToSlot(unit, next, budget - 1);
          else fallbackPlainMove(unit);
        });
        return;
      }
      ...
      if (d < 16) {
        // Genuinely at the slot (same 32u cell) — hold. This is the ranged
        // unit already at range; dragging it anywhere defeats the formation.
        if (config.debugFormation) { const s = formStats(); s.emptyPath++; s.heldAtSlot++; }
        return;
      }
      if (config.debugFormation) formStats().emptyPath++;
      // Same-cell A* quirk: a short hop is fine, but only when it does not cut
      // through a stamped footprint or blocked terrain.
      if (d < 96 && world.pathFinder.hasWorldLineOfSight(unit.currentX, unit.currentY, slot.x, slot.y)) {
        unit.moveTo(world, { isDifferentSpot: true, walkPath: [{ x: slot.x, y: slot.y, weight: 15 }] },
          slot.x, slot.y);
        return;
      }
      // Unreachable slot — try another side of the ring, then give up into a
      // plain move (never a silent stop, never a through-the-wall hop).
      if (budget > 0 && attackCtx && attackCtx.mode === 'surround') {
        if (config.debugFormation) formStats().retried++;
        const next = CombatFormation.claimCandidate(
          attackCtx.candidates, unit.currentX, unit.currentY, attackCtx.shape);
        if (next) return dispatchToSlot(unit, next, budget - 1);
      }
      fallbackPlainMove(unit);
    };

    const moveToSlot = (unit, slot) => dispatchToSlot(unit, slot, SLOT_RETRY_BUDGET);
    ...
    Object.values(groups).forEach(group => {
      ...
      // Combat formation slots (concave or siege surround) were resolved once
      // over the whole army above; each unit either has a slot or falls
      // through to the normal move/formation path. Dispatch itself is the
      // shared dispatchToSlot above the group loop.
      ...
      if (groupUnits.length <= 1 || groupUnits[0].isTransport) {
        ...
        groupUnits.forEach(unit => {
          ...
          if (attackSlots && attackSlots.has(unit)) {
            if (config.debugFormation) formStats().applied++;
        ...
        groupUnits.forEach((unit, idx) => {
          unit.currentGroupId = groupId;
          // Attack-kind: per-unit slot (concave arc or siege surround).
          if (attackSlots && attackSlots.has(unit)) {
            if (config.debugFormation) formStats().applied++;
            moveToSlot(unit, attackSlots.get(unit));
            return;
          ...
          if (idx === 0) {
            ...
            if (!unitWalkInfo.walkPath.length) {
              // Combat orders never copy the leader's polyline — a unit far
              // from the leader would walk the leader's route from its own
              // position. It routes itself to the raw target instead.
              if (combatOrder) {
                const own = world.pathFinder.findPath(unit.currentX, unit.currentY, targetX, targetY);
                if (own.walkPath.length) {
                  unit.moveTo(world, own, targetX, targetY);
                  return;
                }
              }
              // Plain moves keep the shared-path fallback (pack movement).
              if (walkInfo.walkPath.length) {
  ...
  static findStartPosition (player, targetX, targetY) {
    ...
    const baseMapName = normalizeMapName(mapName);

    // Same resolver PlayerManager.setGridData used to pick the terrain, so a
    // player's start position can never come from a different map than the
    // one being simulated. When these disagreed the bases spawned on a map
    // that was never loaded — on Echo Isles S2 that was 1536 units of Y.
    const mapDataKey = resolveMapKey(mapName, mapDataByFile);
    let startPositions = mapDataKey ? mapStartPositions[mapDataKey] : null;

    // Legacy alias table, consulted only when the resolver has nothing. Maps
    // a config key to the plain ladder filename some hosts report.
    if (!startPositions) {
      ...
      const commonMapName = Object.keys(commonMapNames).find(mapKey => {
        return baseMapName.indexOf(commonMapNames[mapKey].toLowerCase()) !== -1;
      });
  ...
  static doAbilityWithTargetAndObjectId (
    ...
    //
    // Resolve the click target to a neutral, and only to a neutral it can
    // actually be. Two guards, because a wrong answer here is expensive: a
    // neutral hit short-circuits the enemy/own-building handling further down
    // (repair, burrow, tree harvest) AND raises a camp interaction event, so an
    // aliased creep both swallows real orders and floods its camp with fake
    // clearing work.
    //
    const neutralByIds = player.world.findNeutralByObjectIds(objectId1, objectId2);
    let foundUnit = null;

    if (neutralByIds) {
      // These ids are bound to a neutral — but a binding can be wrong (see
      // World.isPlausibleNeutralTarget). Neutrals are static, so a click
      // landing far from it was aimed at something else; treat it as a
      // non-neutral click and never re-bind the ids.
      if (player.world.isPlausibleNeutralTarget(neutralByIds, targetX, targetY)) {
        foundUnit = neutralByIds;
      } else {
        console.logger(`ignoring implausible neutral hit ${neutralByIds.displayName} ` +
          `for object ids [ ${objectId1}, ${objectId2}] at (${targetX}, ${targetY})`);
      }
    } else if (!player.world.findUnitByObjectIds(objectId1, objectId2)) {
      // Unowned handles: nothing in the world claims them, so guessing the
      // nearest unregistered neutral at the click point is reasonable. If a
      // registered player unit DOES own them the click targeted that unit, and
      // guessing would alias a creep to it for the rest of the replay.
  ...
  static _resolveCombatFormation (player, world, moveUnits, targetX, targetY, opts) {
    ...
    const enemyHits = world.findEnemyUnitsNear(
      targetX, targetY, searchRadius, player.id, player.teamId, true
    );
    const mobileHits = enemyHits.filter(h => !h.unit.isBuilding);
    const buildingHits = enemyHits.filter(h => h.unit.isBuilding);

    if (!mobileHits.length) {
      // No mobile enemy near the destination.
      //  - a building there makes it a SIEGE: surround the building's real
      //    footprint (the clicked building wins; else the nearest one).
      //  - attack-move onto truly empty ground is just an advance → no
      //    formation (plain move).
      //  - attack-on-unit with an unregistered target: still give ranged
      //    units a range stop around the clicked point so they don't walk on
      //    top of it.
      const clicked = (opts.targetUnit && opts.targetUnit.isBuilding) ? opts.targetUnit : null;
      const targetBuilding = clicked || (buildingHits.length ? buildingHits[0].unit : null);
      if (targetBuilding && targetBuilding.currentX != null && targetBuilding.currentY != null) {
        const entry = (world.collisionWorld && world.collisionWorld.getPathingEntry)
          ? world.collisionWorld.getPathingEntry(targetBuilding.itemId)
          : null;
        const shape = CombatFormation.buildingShape(targetBuilding, entry);
        const walkTest = world.pathFinder
          ? (x, y) => !world.pathFinder.isWorldBlocked(x, y)
          : null;
        const res = CombatFormation.resolveSurround(units, shape, player, walkTest, opts);
        if (config.debugFormation) {
          PlayerActions._recordFormationTrace(
            player, units, { x: shape.x, y: shape.y }, [targetBuilding], res.slots, opts
          );
        }
        return { slots: res.slots, candidates: res.candidates, shape: res.shape, mode: 'surround' };
      }
      if (opts.kind === 'attack') {
        return {
          slots: CombatFormation.resolveFormation(units, { x: targetX, y: targetY }, [], player),
          mode: 'concave'
        };
      }
      return null;
    ...
    // Lone mobile target mobbed by melee: WC3 forms a ring around it (the
    // surround), not a one-sided line. Gated narrowly — a single enemy, at
    // least 3 melee attackers — so ordinary army-vs-army orders keep the
    // concave exactly as before.
    if (mobileHits.length === 1) {
      const lone = mobileHits[0].unit;
      const meleeCount = units.filter(u =>
        (getEffectiveRange(u.itemId, player) || 0) <= CombatFormation._constants.MELEE_MAX_RANGE
      ).length;
      if (meleeCount >= 3 && lone.currentX != null && lone.currentY != null) {
        const shape = {
          x: lone.currentX, y: lone.currentY,
          radius: (lone.balanceInfo && lone.balanceInfo.collisionSize) || 16,
          kind: 'circle'
        };
        const walkTest = world.pathFinder
          ? (x, y) => !world.pathFinder.isWorldBlocked(x, y)
        ...
        if (config.debugFormation) {
          PlayerActions._recordFormationTrace(
            player, units, { x: shape.x, y: shape.y }, [lone], res.slots, opts
          );
        }
        return { slots: res.slots, candidates: res.candidates, shape: res.shape, mode: 'surround' };
      }
    ...
    const enemyUnits = mobileHits.map(h => h.unit);
