...
const PlayerActions = class {
  ...
  static findStartPosition (player, targetX, targetY) {
    ...
    if (mapStartPositions[baseMapName]) {
      ...
      const commonMapName = Object.keys(commonMapNames).find(mapKey => {
        const mapItem = commonMapNames[mapKey];

        const basMapNameLower = baseMapName.toLowerCase();
        const mapItemLower = mapItem.toLowerCase();

        if (basMapNameLower.indexOf(mapItemLower) !== -1) {
          return mapKey;
        }
      });
  ...
  static doAbilityWithTargetAndObjectId (
    ...
    if (foundUnit) {
      ...
      // track workers being sent to gold mine
      if (foundUnit.isGoldmine) {
        const workers = player.getSelectionUnits().filter(u => u.meta.worker);
        workers.forEach(() => {
          if (player.workerState.onLumber > 0) {
            player.workerState.onLumber--;
            player.workerState.onGold++;
          }
        });
      }
