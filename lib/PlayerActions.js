...
const PlayerActions = class {
  ...
  static moveSelectedUnits (
    ...
    Object.values(groups).forEach(group => {
      ...
      // Attack-kind override: instead of every unit converging on (targetX,
      // targetY), distribute them across a concave arc at each unit's own
      // attack range. Ranged units stop at their range; melee push closer.
      // No effect for move/smart commands — those still use the original
      // formation offsets logic below.
      const isAttack = (opts.kind === 'attack');
      const attackSlots = isAttack
        ? PlayerActions._computeAttackSlots(groupUnits, targetX, targetY, player)
        : null;
      ...
      if (groupUnits.length <= 1 || groupUnits[0].isTransport) {
        ...
        groupUnits.forEach(unit => {
          ...
          if (attackSlots && attackSlots.has(unit)) {
            const slot = attackSlots.get(unit);
            const wi = world.pathFinder.findPath(unit.currentX, unit.currentY, slot.x, slot.y);
            unit.moveTo(world, (wi.walkPath.length ? wi : walkInfo), slot.x, slot.y);
          } else {
        ...
        groupUnits.forEach((unit, idx) => {
          ...
          if (attackSlots && attackSlots.has(unit)) {
            const slot = attackSlots.get(unit);
            let wi = world.pathFinder.findPath(unit.currentX, unit.currentY, slot.x, slot.y);
            if (!wi.walkPath.length && walkInfo.walkPath.length) {
              wi = { isDifferentSpot: walkInfo.isDifferentSpot, walkPath: walkInfo.walkPath.slice() };
            }
  ...
  // ---- Range-aware attack slot distribution ---------------------------------
  //
  // For an "attack" action against (targetX, targetY), compute each unit's
  // final stop position. Two effects combined:
  //   1. Range stop: a unit with attack range R stops at distance R from the
  //      target instead of walking on top of it. Melee (range ~100) close in;
  //      ranged (range 400-600) hang back.
  //   2. Concave arc: multiple attackers fan across an angular spread around
  //      the group's approach vector, so they don't stack on a single ray.
  //
  // Returns Map<unit, { x, y }>. Caller pathfinds each unit to its slot.

  static _computeAttackSlots (units, targetX, targetY, player) {
    const slots = new Map();
    if (!units || !units.length) return slots;

    // Group centroid → direction the army is coming FROM (away from target).
    let cx = 0, cy = 0, n = 0;
    for (const u of units) {
      if (u.currentX == null || u.currentY == null) continue;
      cx += u.currentX; cy += u.currentY; n++;
    }
    if (!n) return slots;
    cx /= n; cy /= n;

    let dx = cx - targetX, dy = cy - targetY;
    const dist = Math.sqrt(dx * dx + dy * dy);
    if (dist > 1e-3) { dx /= dist; dy /= dist; }
    else             { dx = 1; dy = 0; }   // group ON target — arbitrary

    // Per-unit effective range. Melee units have very low values (~90); clamp
    // to MIN_STOP so they don't share coordinates with the target.
    const MIN_STOP = 64;
    const ranges = units.map(u => {
      const r = getEffectiveRange(u.itemId, player) || 0;
      return Math.max(MIN_STOP, r);
    });

    // Angular spread: ~22 deg per unit, capped at ~150 deg total so a 10-
    // unit blob doesn't wrap around the target like a horseshoe.
    const STEP_DEG = 22;
    const MAX_ARC_DEG = 150;
    const arcDeg = Math.min((units.length - 1) * STEP_DEG, MAX_ARC_DEG);
    const stepRad = (units.length > 1) ? (arcDeg * Math.PI / 180) / (units.length - 1) : 0;
    const startRad = -((arcDeg * Math.PI / 180) / 2);

    units.forEach((u, i) => {
      const angle = startRad + stepRad * i;
