...
const PlayerActions = class {
	...
	static findStartPosition (player, targetX, targetY) {
		...
		const baseMapName = path.basename(mapName);
		const startPositions = mapStartPositions[baseMapName.toLowerCase()];

		if (!startPositions) {
			console.error("unrecognized map: unable to register start positions", mapStartPositions);
		}
