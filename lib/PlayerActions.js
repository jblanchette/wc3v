...
const PlayerActions = class {
	...
	static registerTabSwitch (
		...
		if (switchedUnit) {
			...
			console.log("WARNING: unable to find tab switch unit.", objectId1, objectId2);

			const switchUnitByItemId = player.findUnregisteredUnitByItemId(fixedItemId);
			if (switchUnitByItemId) {
				console.log("found switch unit to register: ", switchUnitByItemId);
				console.log("Registering objectIds to unit: ", objectId1, objectId2);
		...
		let unitSelectionIndex = player.units.findIndex(sUnit => {
			return sUnit.itemId1 === finalSwitchedUnit.itemId1 &&
			       sUnit.itemId2 === finalSwitchedUnit.itemId2;
		});

		if (unitSelectionIndex === -1) {
			console.log("final unit: ", finalSwitchedUnit);
			console.log("raw sel: ", player.selection.units);

			console.log("index? ", unitSelectionIndex);

			throw new Error("unable to find unit in selection to tab switch to.");
		}
