...
const PlayerActions = class {
  ...
  static moveSelectedUnits (
    ...
    Object.values(groups).forEach(group => {
      ...
      const hasNonWorker = groupUnits.find(unit => {
        return !unit.meta.worker;
      });

      if (!hasNonWorker) {
        console.logger("not counting netural group with only workers.");
  ...
  static findStartPosition (player, targetX, targetY) {
    ...
    let baseMapName = path.basename(mapName).toLowerCase();
