...
const Hero = class extends Unit {
  ...
  static castSummon (
    ...
    for (let i = 0; i < summonCount; i++) {
      ...
      // Provenance. A WC3 summon spawns already aggressive — it acquires and
      // chases without any order — but it has no path and no orders of its own
      // until the player selects it, so the viewer had nothing to say it was in
      // the fight and animated it standing still. Recording who summoned it and
      // when lets the client inherit the caster's combat context.
      summonUnit.summonedByUuid = focusUnit.uuid;
      summonUnit.summonTime = player.eventTimer.timer.gameTime;
          ...
          switch (itemData.type) {
            case "summon":
              ...
              for (let i = 0; i < summonCount; i++) {
                ...
                // Same provenance as the spell path. This is the Rod of
                // Necromancy case (rnec -> 2x uske, 65s): the skeletons appear
                // mid-battle with no orders of their own and used to animate
                // idle for their entire life.
                newUnit.summonedByUuid = focusUnit.uuid;
                newUnit.summonTime = player.eventTimer.timer.gameTime;
