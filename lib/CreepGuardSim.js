...
// Movement is collision-checked against the SAME pathing grid the A* uses for
// player units (PathFinder.isWorldBlocked: unwalkable terrain, tree doodads and
// stamped building footprints). Two details matter:
//
//   - A single sim step can be ~90wu at creep speed while a tree blocks a 128wu
//     square, so the step is subdivided to at most half a pathing cell. Testing
//     only the endpoint let creeps hop straight over a trunk.
//   - The creep's body has width, so the destination is tested at the centre
//     AND around the collision disc, mirroring CollisionWorld.isFree's static
//     half. Centre-only checks let a creep clip its shoulder through a tree.
//
// Dynamic unit-vs-unit collision is deliberately NOT applied: neutral creeps
// are not in CollisionWorld's dynamic tree, and adding them would change how
// every PLAYER unit resolves its own position, rewriting paths across the whole
// corpus. Creeps therefore respect terrain, not other units.
const MAX_SUBSTEP_WU = 16;
const BODY_SAMPLES = 8;

// Breadcrumb spacing for the walk home. A creep that chased around an obstacle
// cannot always get back in a straight line — the outbound route only worked
// because of axis sliding — so it retraces its own trail instead. That is
// guaranteed passable and guaranteed inside the leash, because the creep
// already walked it, which neither a straight line nor a fresh A* detour can
// promise (an A* route around a tree can bulge past the leash circle).
// Measured: without this, 716 creeps across the corpus ended the replay parked
// in open ground on camps that were never cleared.
const TRAIL_SPACING_WU = 64;
...
const CreepGuardSim = class {
  constructor (world, wc3vPlayers) {
    ...
    this.pathFinder = (world && world.pathFinder) || null;
    this.stats = {
      camps: 0, campsSimulated: 0, creepsMoved: 0,
      samples: 0, leash: null, leashSource: null,
      // terrain accounting. blockedSteps = times a creep was stopped or had to
      // slide along an obstacle (proves collision is doing something);
      // blockedSamples = emitted positions that sit on blocked terrain, which
      // must be ZERO — it is the "creeps walking through trees" bug, asserted
      // against the same grid the A* uses rather than left to the eye.
      blockedSteps: 0, blockedSamples: 0
    };
  }

  //
  // Can a creep of this body radius stand here without overlapping terrain?
  // Centre plus a ring around the collision disc — see MAX_SUBSTEP_WU above.
  //
  _canStand (x, y, radius) {
    const pf = this.pathFinder;
    if (!pf || typeof pf.isWorldBlocked !== 'function') return true;
    if (pf.isWorldBlocked(x, y)) return false;
    if (radius <= 0) return true;
    for (let i = 0; i < BODY_SAMPLES; i++) {
      const a = (Math.PI * 2 * i) / BODY_SAMPLES;
      if (pf.isWorldBlocked(x + Math.cos(a) * radius, y + Math.sin(a) * radius)) {
        return false;
      }
    }
    return true;
  ...
  //
  // Walk from (x, y) toward (tx, ty), stopping at terrain. Subdivided so a fast
  // creep cannot tunnel through a tree, and sliding along a single axis when the
  // diagonal is blocked — otherwise a creep meeting a tree line stops dead
  // instead of moving around it.
  //
  // `bound` = { x, y, leash }: the guard post and how far from it the creep may
  // ever be. It is enforced PER SUBSTEP rather than by clamping the result,
  // because sliding moves perpendicular to the intended direction and can carry
  // a creep sideways past the leash circle even when its goal was inside it.
  // Enforcing it here makes the leash hold by construction; clamping afterwards
  // could only fix it by shoving the creep back into terrain it just avoided.
  _walkToward (x, y, tx, ty, radius, bound) {
    const total = Math.hypot(tx - x, ty - y);
    if (!(total > 0)) return { x, y, blocked: false };

    const steps = Math.max(1, Math.ceil(total / MAX_SUBSTEP_WU));
    const dx = (tx - x) / steps;
    const dy = (ty - y) / steps;

    const ok = (px, py) => {
      if (bound && dist(px, py, bound.x, bound.y) > bound.leash) return false;
      return this._canStand(px, py, radius);
    };
    ...
    for (let i = 0; i < steps; i++) {
      ...
      // slide: keep whichever single-axis component still clears
      if (ok(nx, cy)) {
        cx = nx;
      } else if (ok(cx, ny)) {
        cy = ny;
  ...
  _simulateCamp (camp, threats, leash) {
    ...
    const sims = creeps.map(creep => {
      ...
      return {
        ...
        // Body radius, from the same field CollisionWorld uses for player units.
        radius: (creep.balanceInfo && creep.balanceInfo.collisionSize) || 0,
        lastSampleT: null,
        lastSampleX: creep.currentX,
        lastSampleY: creep.currentY,
        moved: false,
        // breadcrumbs dropped on the way out, consumed on the way home
        trail: []
      };
    });
    ...
    for (let t = window.start; t <= window.end; t += SIM_STEP_MS) {
      ...
      for (const s of sims) {
        ...
        if (s.mode === 'chase' && (!target || leashUsed >= leash)) {
          ...
          if (dist(s.x, s.y, s.guard.x, s.guard.y) <= ARRIVE_EPSILON_WU) {
            s.mode = 'guard';
            s.trail.length = 0;
          }
        }
        ...
        if (s.mode === 'chase' && target) {
          ...
          // Retrace the breadcrumbs, newest first, then head for the guard post.
          // Drop each crumb once it is reached so the creep keeps unwinding.
          while (s.trail.length &&
                 dist(s.x, s.y, s.trail[s.trail.length - 1].x, s.trail[s.trail.length - 1].y)
                   <= TRAIL_SPACING_WU) {
            s.trail.pop();
          }
          goal = s.trail.length
            ? { x: s.trail[s.trail.length - 1].x, y: s.trail[s.trail.length - 1].y }
            : { x: s.guard.x, y: s.guard.y };
        } else {
          goal = { x: s.x, y: s.y };
        }

        // ── step toward the goal ────────────────────────────────────────
        // The leash is applied to the GOAL, not to the position afterwards:
        // clamping a position could drop a creep inside a tree it had just
        // walked around. Since a step never overshoots its goal, a goal inside
        // the leash keeps the creep inside it.
        const goalFromGuard = dist(goal.x, goal.y, s.guard.x, s.guard.y);
        if (goalFromGuard > leash) {
          const f = leash / goalFromGuard;
          goal = {
            x: s.guard.x + (goal.x - s.guard.x) * f,
            y: s.guard.y + (goal.y - s.guard.y) * f
          };
        }
        ...
        if (gd > 0.5) {
          ...
          const tx = s.x + (goal.x - s.x) * (step / gd);
          const ty = s.y + (goal.y - s.y) * (step / gd);
          const moved = this._walkToward(s.x, s.y, tx, ty, s.radius,
            { x: s.guard.x, y: s.guard.y, leash });
          s.x = moved.x;
          s.y = moved.y;
          if (moved.blocked) this.stats.blockedSteps++;

          // Outbound only: leave a breadcrumb trail to unwind on the way back.
          if (s.mode === 'chase') {
            const tail = s.trail.length ? s.trail[s.trail.length - 1] : null;
            if (!tail || dist(s.x, s.y, tail.x, tail.y) >= TRAIL_SPACING_WU) {
    ...
    // camp that was pulled and survived is not left frozen mid-stride. The
    // guard spot is the map's own spawn point, so it is standable by
    // construction — but walk there rather than teleporting, so the final
    // sample cannot cut a corner through a tree the creep walked around.
    sims.forEach(s => {
      if (!s.moved) return;
      if (dist(s.lastSampleX, s.lastSampleY, s.guard.x, s.guard.y) <= ARRIVE_EPSILON_WU) return;

      // Unwind whatever trail is left, then close the last gap to the guard.
      // Same route the creep came out on, so every leg is known passable.
      const bound = { x: s.guard.x, y: s.guard.y, leash };
      while (s.trail.length) {
        const crumb = s.trail.pop();
        const leg = this._walkToward(s.x, s.y, crumb.x, crumb.y, s.radius, bound);
        s.x = leg.x;
        s.y = leg.y;
      }
      const home = this._walkToward(s.x, s.y, s.guard.x, s.guard.y, s.radius, bound);
      s.x = home.x;
      s.y = home.y;
  ...
  _emit (s, t) {
    ...
    if (!this._canStand(s.x, s.y, s.radius)) this.stats.blockedSamples++;
