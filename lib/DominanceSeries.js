...
class DominanceSeries {
  ...
  run () {
    ...
    for (const p of players) perPlayer.set(p, this._collectStateEvents(p, gameEnd));
    this._collectEndgameLosses(players, perPlayer, gameEnd);
  ...
  _razeQualifies (unit) {
    const rs = unit.razedState;
    if (!rs || !rs.razed) return false;
    const min = (this.config.raze && this.config.raze.confidenceMin) || 'high';
    return rs.confidence === 'high' || min === 'medium';
  }
  ...
  _collectStateEvents (player, gameEnd) {
    const cfg = this.config;
    const data = {
      ...
      endgameLossTimes: [],  // ticks for endgame weighted losses (honest steps)
      staleLossUuids: new Set()   // units already partially subtracted as stale
    };
    ...
    for (const u of (player.units || [])) {
      ...
      if (lossT != null && lossT > spendT) {
        list.push({ t: lossT, delta: -cost });
      } else if (u.isBuilding && this._razeQualifies(u) && u.razedState.at > spendT) {
        // Razed by enemy attack (lib/RazedBuildingInference.js) — standing
        // value ends at the raze verdict.
        list.push({ t: u.razedState.at, delta: -cost });
        data.endgameLossTimes.push(u.razedState.at);
      } else if (!u.isBuilding) {
        // Zombie-unit staleness: lostState is a FINAL verdict, so a
        // sub-threshold possiblyLost means the unit was NEVER seen again
        // after `since`. DeathInference caps mechanism confidence at 40-45
        // for silence-only deaths, but the EVIDENCE strengthens with the
        // silent span — a combat unit permanently silent for minutes of a
        // pro game is almost certainly dead, and pricing it at full value
        // inflates the owner's army. Weight ramps with the silence.
        const sl = cfg.staleLoss;
        const ls = u.lostState;
        if (sl && ls && ls.state === 'possiblyLost' &&
            (ls.confidence || 0) < cfg.possiblyLostConfidenceMin &&
            ls.since != null && gameEnd != null) {
          const silentSpan = gameEnd - ls.since;
          if (silentSpan >= sl.minSilenceMs) {
            const ramp = Math.min(1,
              (silentSpan - sl.minSilenceMs) / Math.max(1, sl.fullSilenceMs - sl.minSilenceMs));
            const w = sl.maxWeight * ramp;
            if (w > 0.01) {
              list.push({ t: ls.since, delta: -cost * w });
              data.staleLossUuids.add(u.uuid);
            }
          }
        }
      }
    ...
    for (const e of (player.eventStream || [])) {
      if (e.key === 'addBuilding' && e.isExpansion) {
        ...
        // A razed expansion stops paying its strategic bonus.
        const expUuid = e.building && e.building.uuid;
        if (expUuid) {
          const expB = (player.units || []).find(u => u.uuid === expUuid);
          if (expB && this._razeQualifies(expB)) {
            data.expBonus.push({ t: expB.razedState.at, delta: -cfg.expansionBonus });
            data.endgameLossTimes.push(expB.razedState.at);
          }
        }
      }
  ...
  // "Absence of evidence is not evidence of survival at file end."
  // DeathInference requires a 60s post-battle silence window before a
  // possiblyDead outcome becomes a confident loss — a concession <60s after
  // the decisive fight leaves its casualties officially "escaped", so the
  // fight that ENDED the game moves the score least. For battles whose
  // observation window is truncated by the end of the replay, weight each
  // possiblyDead unit's loss by the outcome's own confidence × the fraction
  // of the window that was actually observable. Battles with a full window
  // are untouched — mid-game behavior is bit-identical to v2.
  _collectEndgameLosses (players, perPlayer, gameEnd) {
    const cfg = this.config.endgame;
    this._endgameLossByBattle = new Map();   // battleId → Map(teamId → Σ weighted GLE)
    if (!cfg) return;

    const battles = this.world.battles || [];
    const endgameBattles = battles.filter(b =>
      (b.endTime || 0) + cfg.windowMs > gameEnd);
    if (!endgameBattles.length) return;

    // Whose base each endgame battle was fought in. Silence at file end is
    // SYMMETRIC (the winner's army idles in the wreckage after the gg, just
    // as quiet as the loser's corpses) — but position + initiative are not:
    // in a battle inside player P's base, P's never-seen-again units died or
    // fled their own base, while the raiders' silent units are alive holding
    // the field. So: base battle → only the DEFENDER's units carry death
    // weight; open-field battle → symmetric.
    const defenderOf = new Map();   // battle.id → defending player (or null)
    for (const b of endgameBattles) {
      defenderOf.set(b.id, this._baseOwnerOf(b, players));
    }

    // uuid → latest endgame battle outcome (possiblyDead only).
    const latestOutcome = new Map();
    for (const b of endgameBattles) {
      for (const o of (b.unitOutcomes || [])) {
        if (o.status !== 'possiblyDead') continue;
        const prev = latestOutcome.get(o.unitUuid);
        if (!prev || (b.endTime || 0) > (prev.battle.endTime || 0)) {
          latestOutcome.set(o.unitUuid, { outcome: o, battle: b });
        }
      }
    }
    if (!latestOutcome.size) return;

    for (const player of players) {
      const data = perPlayer.get(player);
      for (const u of (player.units || [])) {
        const hit = latestOutcome.get(u.uuid);
        if (!hit) continue;
        // Confident-loss units are already subtracted by the state walk,
        // and stale-silence units already carry a partial subtraction.
        if (this._confidentLossTime(u) != null) continue;
        if (data.staleLossUuids.has(u.uuid)) continue;
        // Mirror the army-pricing filters; heroes deliberately excluded
        // (their value has its own model — see plan note on identity guards).
        if (u.isIllusion || u.isItem || u.isInferred || u.isBuilding) continue;
        if (u.meta && (u.meta.hero || u.meta.worker)) continue;
        if (u.summonDuration) continue;
        const gle = this._unitGLE(u);
        if (gle <= 0) continue;

        const { outcome, battle } = hit;
        // Base battle: only the defender's units carry death weight.
        const defender = defenderOf.get(battle.id);
        if (defender && defender !== player) continue;
        const confW = cfg.confidenceWeights[outcome.confidence];
        if (!confW) continue;
        const lastSeen = outcome.lastSeenTime != null ? outcome.lastSeenTime : (battle.endTime || 0);
        // The discriminator DeathInference uses (its :176): the unit must
        // never have been seen again after the battle. At file end EVERY unit
        // goes silent (the winner stops issuing orders after the gg) — silence
        // alone would penalize the winner's idle army. A unit whose path
        // resumed after the battle demonstrably escaped; only never-seen-again
        // units carry death weight.
        const lastActivity = (u.path && u.path.length)
          ? u.path[u.path.length - 1].gameTime
          : (u.spawnTime || 0);
        if (lastActivity > lastSeen + 5000) continue;
        const observedFrac = Math.min(1, Math.max(0, gameEnd - lastSeen) / cfg.windowMs);
        const weightedLoss = confW * observedFrac * gle;
        if (weightedLoss <= 0) continue;

        const t = battle.endTime || 0;
        data.army.push({ t, delta: -weightedLoss });
        data.endgameLossTimes.push(t);

        let byTeam = this._endgameLossByBattle.get(battle.id);
        if (!byTeam) { byTeam = new Map(); this._endgameLossByBattle.set(battle.id, byTeam); }
        byTeam.set(player.teamId, (byTeam.get(player.teamId) || 0) + weightedLoss);
      }
      data.army.sort((a, b) => a.t - b.t);
    }
  }

  // Which player's base a battle was fought in: the unique player whose
  // starting position or completed expansion sits within BASE_RADIUS of the
  // battle's bbox center. Null when nobody (open field) or both (rare
  // cross-base sprawl — treat as open field, stay symmetric).
  _baseOwnerOf (battle, players) {
    const bb = battle.outerBbox;
    if (!bb) return null;
    ...
    for (const p of players) {
      const spots = [];
      if (p.startingPosition) spots.push(p.startingPosition);
      for (const e of (p.eventStream || [])) {
        if (e.key === 'addBuilding' && e.isExpansion && e.building && e.building.lastPosition) {
          spots.push(e.building.lastPosition);
        }
      }
      const inBase = spots.some(s => {
        const dx = s.x - cx, dy = s.y - cy;
        return dx * dx + dy * dy <= R2;
      });
      if (inBase) {
        if (owner) return null;   // both bases in range — ambiguous
        owner = p;
      }
    }
    return owner;
  ...
  _collectMomentumEvents (players, perPlayer) {
    ...
    // Base razes — tempo credit to the razing team (the victim already takes
    // the structural building/expansion subtraction; no double penalty).
    const razesByBattle = new Map();   // battleId → {victimTeam, gle}
    for (const player of players) {
      for (const u of (player.units || [])) {
        if (!u.isBuilding || !this._razeQualifies(u)) continue;
        const rs = u.razedState;
        let agg = razesByBattle.get(rs.battleId);
        if (!agg) { agg = new Map(); razesByBattle.set(rs.battleId, agg); }
        agg.set(player.teamId, (agg.get(player.teamId) || 0) + this._unitGLE(u));
      }
    }
    if (razesByBattle.size) {
      const battleById = new Map((this.world.battles || []).map(b => [b.id, b]));
      for (const [battleId, byVictimTeam] of razesByBattle) {
        const battle = battleById.get(battleId);
        if (!battle) continue;
        for (const [victimTeam, gle] of byVictimTeam) {
          const delta = Math.min(cfg.baseRazeCap, cfg.baseRazeFactor * gle);
          if (delta <= 0) continue;
          for (const part of (battle.participants || [])) {
            if (part.teamId == null || part.teamId === victimTeam) continue;
            const razer = byId.get(part.playerId);
            if (!razer) continue;
            perPlayer.get(razer).momentum.push({
              t: battle.endTime || 0, kind: 'baseRaze', delta, battleId
            });
          }
        }
      }
    ...
    for (const battle of (this.world.battles || [])) {
      ...
      // Endgame weighted losses (see _collectEndgameLosses) join the swing.
      // No overlap with the definite bucket: those units were skipped there
      // precisely because they lack a confident lostState.
      const endgameByTeam = this._endgameLossByBattle && this._endgameLossByBattle.get(battle.id);
      if (endgameByTeam) {
        for (const [teamId, gle] of endgameByTeam) {
          if (teamId in teamLoss) teamLoss[teamId] += gle;
        }
      }
  ...
  _buildTickList (gameEnd, players, perPlayer) {
    ...
    for (const player of players) {
      ...
      for (const t of data.endgameLossTimes) eventTimes.add(t);
    }
