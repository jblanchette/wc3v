...
const Player = class {
	...
	printUnits () {
		...
		console.log(this.units.map(unit => unit.displayName));
	}
	...
	assignPossibleSelectGroup (itemId) {
		...
		return selectionUnits.reduce((registeredUnitsAcc, selectionUnit) => {
			self.possibleSelectList = self.possibleSelectList.filter(selectItem => {
				...
				console.log("checking pos sel group: ", itemId1, itemId2);
				console.log("sel item: ", selectItem);
				...
				if (foundSelectionUnit) {
					console.log("found unit in selection");
		...
		if (!firstGroupUnit && !newlyRegisteredUnits.length) {
			...
			if (fixedItemId !== firstGroupUnit.itemId) {
				console.log(
					...
					"firstUnitItemId: ", firstGroupUnit.itemId
				);
	...
	useAbilityNoTarget (action) {
		...
		console.log("useAbilityNoTarget, sel units: ", selectedUnits.length);

		if (!selectedUnits.length) {
			console.log("error, no units to use ability?", itemId);
			console.log("selection: ", this.selection.units);

			let existingUnits = this.filterUnitByItemId(itemId);

			console.log("possible units: ", existingUnits);
	...
	useAbilityWithTarget (action) {
		...
		if (isItemArray) {
			...
			console.log("first unit is: ", firstUnit.displayName);
