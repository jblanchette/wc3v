...
const Player = class {
	...
	findUnregisteredUnitByItemId (itemId) {
		console.logger("findUnreg by itemId:", itemId);
	...
	useAbilityWithTarget (action) {
		...
		if (isItemArray) {
			...
			if (workerInGroup) {
				...
				console.logger("moving unit to build: ", building.displayName, targetX, targetY);
