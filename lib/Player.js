...
const Player = class {
	...
	assignPossibleSelectGroup (itemId) {
		...
		const results = selectionUnits.reduce((registeredUnitsAcc, selectionUnit) => {
			self.possibleSelectList = self.possibleSelectList.filter(selectItem => {
				...
				if (foundSelectionUnit) {
					...
					if (foundPlayerUnit) {
						const existingUnitCheck = self.findUnit(itemId1, itemId2);
						if (existingUnitCheck) {
							console.logger("ERROR: already unit at this item id?");
							existingUnitCheck.printUnit();

							throw new Error("ERROR already unit at item id in possibleSelectList");
							return false;
						}

						foundPlayerUnit.performBackfill(backfill);

						console.logger("assign calling registerItemIds:", itemId1, itemId2);
	...
	selectSubgroup (action) {
		...
		console.logger("about to check assignPossibleSelectGroup");
		...
		if (firstGroupUnit) {
			console.logger("jeff first group unit found unit at:", itemId1, itemId2);
			...
			console.logger("jeff nothing for first group unit at:", itemId1, itemId2);
			console.logger("new reg list:", newlyRegisteredUnits);
		}

		const { numberUnits } = this.selection;
		...
		if (!firstGroupUnit && !knownUnitByObjectIds && !newlyRegisteredUnits.length) {
	...
	useAbilityNoTarget (action) {
		...
		if (selectedUnits.length) {
			...
			console.logger("First unit: ", firstUnit.displayName);
		...
		if (isItemArray) {
			...
			switch (abilityActionName) {
				...
				case 'SummonTreants':
					...
					Hero.castSummon(this, firstUnit.getSkillForType("summon"));
				break;
					...
					if (knownItem) {
						...
						if (potentialItem) {
							...
							if (unknownObject) {
								...
								let newUnknownItem = new Item(this.eventTimer, null, null, null, false);
