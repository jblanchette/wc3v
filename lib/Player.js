...
const Player = class {
  ...
  //
  // True once `itemId` research has finished — start time plus its build time,
  // from researchMeta. researchLevels alone only says it was STARTED, which is
  // not enough to license an ability the research unlocks.
  //
  hasCompletedResearch (itemId, gameTime) {
    const meta = researchMeta[itemId];
    for (const entry of this.researchStream) {
      if (entry.itemId !== itemId) continue;
      const seconds = (meta && meta.time && meta.time[(entry.level || 1) - 1]) || 0;
      if (entry.gameTime + seconds * utils.SECONDS_TO_MS <= gameTime) return true;
    }
    return false;
  }

  //
  // Apply a form-morph ability (currently only Destroyer Form) to a selection.
  //
  // The order is sent ONCE for the whole selection, so a player morphing three
  // statues produces one action affecting three units — walking only the
  // subgroup leader would under-count every multi-statue morph.
  //
  // Units that aren't in the source form are skipped, which keeps a mixed
  // selection (statues + destroyers + ghouls) from morphing the wrong things.
  //
  applyUnitMorph (units, spellInfo) {
    const { morph, toggle } = spellInfo;
    if (!morph) return [];

    // toggle 'off' is the revert order (unavengerform): destroyer → statue.
    const fromItemId = toggle === 'off' ? morph.on : morph.off;
    const toItemId   = toggle === 'off' ? morph.off : morph.on;
    const gameTime   = this.eventTimer.timer.gameTime;

    // A pressed button is not a completed morph. The replay records the order
    // whether or not the game accepted it, so gate on the prerequisite research
    // actually being FINISHED — otherwise an early click turns a statue into a
    // Destroyer that never existed.
    if (morph.requiresResearch &&
        !this.hasCompletedResearch(morph.requiresResearch, gameTime)) {
      console.logger(`player ${this.id} morph rejected — ${morph.requiresResearch} not complete`);
      return [];
    }

    const morphed = [];
    (units || []).forEach(unit => {
      if (!unit || unit.itemId !== fromItemId) return;
      if (unit.morphTo(toItemId, gameTime)) morphed.push(unit);
    });

    if (morphed.length) {
      console.logger(`player ${this.id} morphed ${morphed.length} ${fromItemId} → ${toItemId}`);
    }
  ...
  //
  getSelectedUnit () {
    // No selection yet is a real state, not an error: a player can act before
    // ever selecting anything. getSelectionUnits one function up already says
    // so and returns empty; this one destructured straight through and threw,
    // which killed the rest of the parse rather than the action.
    if (!this.selection) {
      console.logger("no selection, no selected unit");
      return null;
    }
  ...
  selectSubgroup (action) {
    ...
    // A subgroup select before this player has ever had a selection. The
    // empty-selection recovery just below cannot help: it repairs the
    // selection by clearing and re-adding into it, and there is no object to
    // do that to. So take the same exit the function already uses when it
    // cannot find a first group item, which is the identical situation with
    // one less indirection.
    if (!this.selection) {
      console.logger("WARNING - Player.selectSubgroup: no selection to read a subgroup from");
    ...
    if (firstGroupUnit && 
      ...
      if (fixedItemId !== firstGroupUnit.itemId) {
        ...
        // morphTo (not a bare assignment) so displayName / meta / combat /
        // movement follow the new form — a raw itemId write leaves a morphed
        // Destroyer reporting itself as an Obsidian Statue.
        firstGroupUnit.morphTo(fixedItemId, this.eventTimer.timer.gameTime);
    ...
    if (!firstGroupUnit && !knownUnitByObjectIds && !newlyRegisteredUnit) {
      ...
      if (upgradeMatches) {
        // same as the morph path above — identity must be rebuilt, not just
        // the itemId swapped, or the upgraded building keeps the old name.
        firstGroupUnit.morphTo(fixedItemId, this.eventTimer.timer.gameTime);
  ...
  changeSelection (action) {
    ...
    // Was the previous change action, in this same tick, a DESELECT? That is
    // the signature of a click-to-reselect: WC3 clears by naming the units it
    // is removing and then names the new ones. A SELECT with no deselect in
    // front of it in the same tick is a shift-add. See the combine rule below.
    const afterDeselectInBlock = sameChangeBlock &&
      this.lastChangeSelectMode === SelectModes.deselect;
    this.lastChangeSelectMode = selectMode;
    ...
    // How a selectMode=1 action combines with the existing selection.
    //
    // A bare SELECT is ambiguous in the protocol: WC3 emits it both for
    // "shift-add these" and for "my selection is now exactly these". A click
    // to reselect prefixes it with a DESELECT naming the old units; a
    // shift-click sends it alone; a drag-select has been observed doing both.
    // Nothing in the action distinguishes the cases, so the rule is empirical.
    //
    // Legacy replaced whenever a SELECT arrived at a gameTime different from
    // the previous change action. That handles click-to-reselect by accident
    // (the deselect shares the click's gameTime, so the SELECT reads as
    // same-block and merges into an already-emptied selection) and gets every
    // SHIFT-ADD wrong: the units already selected are silently evicted, and
    // orders miss them until something re-adds them. Graded against the
    // AssignGroupHotkey oracle (Ctrl+N states the true selection), legacy
    // misses units on 20-46% of samples depending on the replay. The visible
    // symptom is a hero dropping out of its army's orders, going stale, then
    // being yanked across the map when a later action puts it back.
    //
    // Merging unconditionally trades that for the opposite failure: WC3 only
    // deselects units it believes are selected, so a unit that drifts in
    // wrongly is never named in a deselect and sticks for the rest of the
    // game, collecting every order the player issues.
    //
    // config.selectionRule picks the rule; config/config.js documents the
    // candidates and tools/order-trace.js --selcheck grades them. Nothing here
    // decides which is right — the oracle does.
    if (this.selection === null) {
      this.selection = subGroup;
    } else {
      let replace;
      switch (config.selectionRule) {
        case 'merge':
          replace = false;
          break;
        case 'merge-resync':
          replace = afterDeselectInBlock;
          break;
        case 'add-single':
          // A shift-click names exactly one unit; anything longer is the game
          // restating the whole selection (drag-select, select-all-of-type).
          replace = afterDeselectInBlock || subActions.length > 1;
          break;
        case 'legacy':
        default:
          replace = !sameChangeBlock && selectMode === SelectModes.select;
          break;
      }
  ...
  //
  // Call to Arms: peasants become militia in place. Same unit instance, same
  // objectIds, new form — exactly what Unit.morphTo models (and it records
  // morphHistory, so the client can draw a peasant before the conversion and a
  // militia after instead of relabelling the whole life of the unit).
  //
  // The auto-expiry matters as much as the conversion. Militia reverts on its
  // own after `evolution.time` seconds whether or not the player presses Back
  // to Work, and that reversion generates NO replay action — so without a
  // timer the unit stays a militia forever, which is how a peasant ends up
  // standing in the woods long after the fight it was called for.
  //
  // Returns true when at least one unit converted (so the caller can swallow
  // the action; a town-hall-issued order with no registered workers selected
  // falls through to the normal dispatch).
  //
  convertMilitia (selectedUnits) {
    const now = this.eventTimer.timer.gameTime;
    let converted = 0;

    for (const unit of selectedUnits) {
      if (!unit || unit.isBuilding) continue;
      const evo = unit.meta && unit.meta.evolution;
      if (!evo || !evo.itemId) continue;                 // not a convertible worker
      const fromItemId = unit.itemId;                    // the form to return to
      if (!unit.morphTo(evo.itemId, now)) continue;      // already militia

      this._traceTask(unit, WorkerTask.MILITIA, 'militia');
      unit.currentTask = WorkerTask.MILITIA;

      // Combat licence. A militia acquires and attacks with no further order,
      // so the replay records nothing after the conversion — and the viewer
      // needs corroboration or it animates the unit standing still through the
      // fight it was called into. The conversion IS the record. Consumed by
      // UnitBehavior's ORDER_WINDOWS.militia.
      unit.recordOrder(now, 'militia', { x: unit.currentX, y: unit.currentY });

      // Cancel a stale expiry before arming a new one — re-converting during
      // an active militia window restarts the clock, it does not stack.
      if (unit._militiaExpiry && !unit._militiaExpiry.completed) {
        this.eventTimer.cancelEvent(unit._militiaExpiry);
      }
      unit._militiaExpiry = this.eventTimer.addEvent(
        (evo.time || 45) * 1000,
        () => {},
        (finished) => {
          if (!finished) return;                          // cancelled by Back to Work
          this.revertMilitia([unit], fromItemId);
        }
      );
      converted++;
    }
  ...
  //
  // Back to Work, or the militia timer running out. `fallbackItemId` is the
  // form to return to when the unit's own meta can't say (the militia form has
  // no `evolution` entry pointing back at the peasant).
  //
  revertMilitia (selectedUnits, fallbackItemId = 'hpea') {
    const now = this.eventTimer.timer.gameTime;
    let reverted = 0;

    for (const unit of selectedUnits) {
      if (!unit || unit.isBuilding) continue;
      if (unit.currentTask !== WorkerTask.MILITIA && !unit._militiaExpiry) continue;

      if (unit._militiaExpiry && !unit._militiaExpiry.completed) {
        this.eventTimer.cancelEvent(unit._militiaExpiry);
      }
      unit._militiaExpiry = null;

      if (!unit.morphTo(fallbackItemId, now)) continue;

      // Back to Work ends combat intent outright — 'stop' is the one kind
      // UnitBehavior treats as a countermand, so the militia licence above
      // cannot outlive the form even when the player reverts early.
      unit.recordOrder(now, 'stop', { x: unit.currentX, y: unit.currentY });

      const back = unit.primaryRole || WorkerTask.GOLD;
      this._traceTask(unit, back, 'militiaRevert');
      unit.currentTask = back;
      reverted++;
    }
  ...
  useAbilityNoTarget (action) {
    ...
    // Stop / Hold Position (0xD0004 / 0xD0019). Both were unmapped, so a stop
    // never cancelled an in-flight move and the simulation kept walking units to
    // destinations the player had already called off. Handled up front because
    // they apply to the whole selection regardless of unit class, and neither is
    // a spell — routing them through the spell lookup is what lost them.
    if (isItemArray) {
      const noTargetOrder = utils.findItemIdForObject(itemId, abilityActions);
      if (noTargetOrder === 'StopCommand' || noTargetOrder === 'HoldPosition') {
        const nowGT = this.eventTimer.timer.gameTime;
        const kind = noTargetOrder === 'StopCommand' ? 'stop' : 'holdposition';
        let handled = 0;
        for (const u of selectedUnits) {
          if (!u || u.isBuilding) continue;
          // Cancel the pending move. `checkStateForMove` is the same cancel path
          // a new move order uses, so a stop behaves like any re-order.
          u.checkStateForMove();
          u.recordOrder(nowGT, kind, { x: u.currentX, y: u.currentY });
          handled++;
        }
        // Only swallow the action if it actually applied to a mobile unit. A
        // building selection falls through to the normal dispatch — cancel-train
        // is keyed off abilityFlags, not this order id, but not eating the
        // action costs nothing and keeps the building path exactly as it was.
        if (handled) return;
      }

      // Call to Arms / Back to Work. See the abilityActions comment: all four
      // order ids were unmapped, so a militia conversion changed nothing and
      // the peasant stood in place for the whole fight.
      if (noTargetOrder === 'Militia' || noTargetOrder === 'MilitiaConvert') {
        if (this.convertMilitia(selectedUnits)) return;
      }
      if (noTargetOrder === 'MilitiaOff' || noTargetOrder === 'MilitiaUnconvert') {
        if (this.revertMilitia(selectedUnits)) return;
      }

      // Resume Harvesting: the worker goes back to whatever it was gathering.
      // The engine re-issues the harvest itself, so no move order follows in
      // the replay — without this the worker keeps whatever task it had (often
      // 'build' or 'militia') and the viewer's harvest loop never picks it up.
      if (noTargetOrder === 'ResumeHarvesting') {
        const gt = this.eventTimer.timer.gameTime;
        let resumed = 0;
        for (const u of selectedUnits) {
          if (!u || u.isBuilding || !u.meta || !u.meta.worker) continue;
          u.setWorkerRole(u.primaryRole || WorkerRole.GOLD, gt, true);
          resumed++;
        }
        if (resumed) return;
      }

      // Burrow / Unburrow. A burrowed crypt fiend is stationary and hidden;
      // unmapped, the simulation happily kept walking it to wherever it was
      // last sent. Burrow cancels the in-flight move exactly like Stop does.
      if (noTargetOrder === 'Burrow' || noTargetOrder === 'Unburrow') {
        const gt = this.eventTimer.timer.gameTime;
        const burrowing = noTargetOrder === 'Burrow';
        let handled = 0;
        for (const u of selectedUnits) {
          if (!u || u.isBuilding) continue;
          u.isBurrowed = burrowing;
          if (burrowing) {
            u.checkStateForMove();
            u.currentTask = WorkerTask.BURROW;
            // 'stop' is the kind UnitBehavior treats as a countermand, which is
            // exactly right: a burrowed unit is not walking and not swinging.
            u.recordOrder(gt, 'stop', { x: u.currentX, y: u.currentY });
          } else if (u.currentTask === WorkerTask.BURROW) {
            u.currentTask = u.primaryRole || WorkerTask.IDLE;
          }
          if (!u.burrowStream) u.burrowStream = [];
          u.burrowStream.push({ gameTime: gt, isBurrowed: burrowing });
          handled++;
        }
        if (handled) return;
      }
      ...
      if (firstUnit.meta.hero) {
        ...
        if (spellInfo) {
          if (spellInfo.isFormToggle) {
            // Destroyer Form swaps the unit's itemId outright; Stone Form /
            // Defend / Bear Form only change stats, so they fall through with
            // just the event. Apply BEFORE exporting the unit reference so the
            // event carries the post-morph name.
            const morphed = this.applyUnitMorph(selectedUnits, spellInfo);
            ...
            this.addEvent('formToggle', {
              ...
              isUnitSpell: true,
              ...(morphed.length ? { morphedCount: morphed.length } : {})
            });
  ...
  useAbilityWithTargetAndObjectId (action) {
    ...
    switch (abilityActionName) {
      ...
      break;
      // Repair (0xD0038). Human peasants repairing a damaged building, which
      // during a fight is most of what the ones not fighting are doing. It was
      // unmapped, so the order fell through the spell lookup and did nothing:
      // the peasant never walked to the building and stood wherever it happened
      // to be. Measured 32 of 32 dropped on the reported replay.
      //
      // The unit walks to the target and stays there, so this is a plain move
      // plus the task change. No combat order is recorded — repairing is not
      // fighting, and claiming otherwise would let a repair licence a swing.
      case 'Repair': {
        (units || []).forEach(unit => {
          if (!unit || unit.isBuilding) return;
          if (!unit.meta || !unit.meta.worker) return;
          unit.currentTask = WorkerTask.REPAIR;
          this._traceTask(unit, WorkerTask.REPAIR, 'repair');
        });
        // Deliberately NOT kind:'move'. That is the strict point-move contract
        // ("must reach the commanded X/Y"), and a repair target is a building
        // the peasant stops at the edge of. Grading it strictly would book a
        // permanent error against every repair.
        PlayerActions.moveSelectedUnits(this, targetX, targetY, { skipScoutDetection: true });
      }
      break;
      case 'MoveCommand':
      ...
      break;
      case 'Patrol':
        // Patrol (0xD0016) walks to the point and engages on the way, so it is
        // an attack-move for both movement and intent. Previously unmapped: the
        // action was dropped and the units simply never went anywhere.
        PlayerActions.moveSelectedUnits(this, targetX, targetY, {
          kind: 'attackground', orderKind: 'patrol'
        });
      break;
      case 'AttackCommand':
      // Explicit attack-ground (0xD0010, mortar/artillery) and attack-once
      // (0xD0011). Both were unmapped until now and fell through to the spell
      // lookup, which misses — so they logged an unknown action and the units
      // never moved. Same dispatch as AttackCommand; the order KIND differs.
      case 'AttackGround':
      case 'AttackOnce':
      case 'RightClick':
        ...
        if (firstUnit && firstUnit.isBuilding) {
          ...
          // Resolve the clicked enemy FIRST. It feeds both the combat signal and
          // the unit's order stream, and the order stream is what lets the viewer
          // say "this unit was told to attack THAT unit" instead of re-deriving a
          // target from geometry every frame.
          const enemyHit = groundClick
            ? null
            : this.world.findEnemyByObjectIds(objectId1, objectId2, this.id, this.teamId);
          ...
          //
          // The MOVEMENT kind stays a 3-value space so formation slots and path
          // timing are untouched. What is new is that the A-click vs right-click
          // distinction is no longer thrown away: it rides along as orderKind, so
          // an explicit attack order is distinguishable from a smart-click.
          const moveKind = groundClick ? 'attackground' : 'attack';
          const explicitAttack = (abilityActionName === 'AttackCommand' ||
                                  abilityActionName === 'AttackGround' ||
                                  abilityActionName === 'AttackOnce');
          let orderKind;
          if (abilityActionName === 'AttackOnce') orderKind = 'attackonce';
          else if (groundClick) orderKind = explicitAttack ? 'attackground' : 'smartground';
          else orderKind = explicitAttack ? 'attack' : 'smartunit';

          PlayerActions.moveSelectedUnits(this, targetX, targetY, {
            kind: moveKind,
            orderKind,
            targetUuid: enemyHit && enemyHit.unit ? enemyHit.unit.uuid : null,
            // The resolved unit object rides along so a click on an enemy
            // BUILDING can anchor the siege surround — the uuid alone is
            // never looked back up in the move path.
            targetUnit: enemyHit ? enemyHit.unit : null
          });
      ...
      case 'HeroRevive':
        ...
        if (targetHero) {
          ...
          this.addEvent('heroRevive', {
            ...
            unit: firstUnit.exportUnitReference(),
            // The hero being revived — a revive is ground truth that this hero
            // WAS dead until now. DominanceSeries keys mid-game hero-death
            // detection off this (lostState alone can't: it's an end-of-game
            // verdict, so a revived hero reads active again).
            hero: targetHero.exportUnitReference()
          });
        }
      break;
      case 'HeroMoveItem1':
      ...
      switch (abilityActionName) {
        ...
        case 'HeroItem6': {
          ...
          if (heroItem) {
            ...
            // No registered unit in the selection — there is nothing to
            // attribute the use to. This used to fall into the branch below
            // and throw on firstUnit.exportUnitReference(), which aborts the
            // whole timeslot loop in wc3v.js: one bad action silently ended
            // the parse. (Measured on 1057610942_Moon_Soin_AutumnLeaves20:
            // the action stream stopped at 12:58 of a 17:26 game, and the
            // validator still reported 0.99 confidence.)
            this.reduceParseConfidence('Tiny');
          } else {
  ...
  assignGroupHotkey (action) {
    ...
    // oldSelection is null when a hotkey group is assigned before this player
    // has ever had a selection. Real and rare: one replay in the shipped
    // corpus (2287505659_Life_Happy_Springtime-13) opens with a Ctrl+N, and
    // the throw took the ENTIRE remaining parse with it, leaving a summary
    // with no players at all rather than an error anybody would see.
    //
    // An empty array is the right stand-in rather than a special case:
    // ensureValidGroup only reads oldGroup[0], and already handles that being
    // undefined by taking the group type from the NEW selection instead.
    const oldUnits = (oldSelection && oldSelection.units) || [];

    this.selection.units = this.ensureValidGroup(oldUnits, this.selection.units);
