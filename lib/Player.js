...
const Player = class {
	...
	selectSubgroup (action) {
		...
		if (!firstGroupUnit && !newlyRegisteredUnits.length) {
			...
			const objectMismatchCheck = firstGroupUnit.objectId1 &&
				(firstGroupUnit.objectId1 !== objectId1 || firstGroupUnit.objectId2 !== objectId2);

			if (fixedItemId !== firstGroupUnit.itemId || objectMismatchCheck) {
				const unitInfo = mappings.getUnitInfo(fixedItemId);
				console.log("Tab-switch from:", firstGroupUnit.displayName, "to: ", unitInfo.displayName);
	...
	useAbilityWithTarget (action) {
		...
		if (isItemArray) {
			...
			if (workerInGroup) {
				console.log("has worker in group: ", workerInGroup);
			}
