...
const Player = class {
  constructor (id, playerSlot, gameData, eventTimer, world, isNeutralPlayer) {
    ...
    // undead ghoul lumber tracking (ghouls harvest lumber, not acolytes)
    this.ghoulState = {
      onLumber: 0,
      totalGhouls: 0
    };
  ...
  initWorkerState () {
    switch (this.race) {
      case 'O': // Orc: 5 peons, typically 4 gold + 1 lumber
        this.workerState = { onGold: 4, onLumber: 1, onBuild: 0, totalWorkers: 5 };
        break;
      case 'H': // Human: 5 peasants, typically 4 gold + 1 lumber
        this.workerState = { onGold: 4, onLumber: 1, onBuild: 0, totalWorkers: 5 };
        break;
      case 'U': // Undead: 3 acolytes on gold, 1 ghoul on lumber
        this.workerState = { onGold: 3, onLumber: 0, onBuild: 0, totalWorkers: 3 };
        this.ghoulState = { onLumber: 1, totalGhouls: 1 };
        break;
      case 'E': // Night Elf: 5 wisps, typically 4 gold + 1 lumber
        this.workerState = { onGold: 4, onLumber: 1, onBuild: 0, totalWorkers: 5 };
        break;
    }
  }
  ...
  addEvent (key, item) {
    ...
    // for undead, include ghoul lumber workers
    if (this.race === 'U') {
      workers.ghoulsOnLumber = this.ghoulState.onLumber;
    }
  ...
  addPlayerUnit (unit, ignoreEvent = false) {
    ...
    if (unit.meta.worker) {
      this.workerState.totalWorkers++;
      // default: new workers go to gold (most common in competitive play)
      if (!ignoreEvent) {
        this.workerState.onGold++;
      }
    }

    // undead ghouls: track separately (they harvest lumber, not acolytes)
    if (this.race === 'U' && unit.itemId === 'ugho' && !ignoreEvent) {
      this.ghoulState.totalGhouls++;
    }

    // some events are intentionally left out of the event stream
    if (ignoreEvent) {
      return;
    }

    this.addEvent('addUnit', {
  ...
  addPlayerBuilding (unit, ignoreEvent = false) {
    ...
    let existingBuilding = this.units.find(playerUnit => {
      if (playerUnit.isBuilding) {
        console.logger("* checking if existing building matches: ", playerUnit.displayName, playerUnit.currentX, playerUnit.currentY);
      }
    ...
    console.logger("** adding building event", this.uuid, unit.displayName);
  ...
  useAbilityWithTarget (action) {
    ...
    if (workerInGroup) {
      ...
      // track worker going to build
      this.workerState.onBuild++;
      if (this.workerState.onGold > 0) {
        this.workerState.onGold--;
      } else if (this.workerState.onLumber > 0) {
        this.workerState.onLumber--;
      }
      ...
      firstUnit.moveToBuild(this.world, building, (eventFinished, isCancelled = false) => {
        ...
        if (!eventFinished) {
          ...
          if (distanceLeft > maxDistanceBuffer) {
            ...
            // worker build cancelled, return to gold
            playerInstance.workerState.onBuild = Math.max(0, playerInstance.workerState.onBuild - 1);
            playerInstance.workerState.onGold++;
            return;
          }
        }
        ...
        // worker finished building, return to gold
        playerInstance.workerState.onBuild = Math.max(0, playerInstance.workerState.onBuild - 1);
        playerInstance.workerState.onGold++;
  ...
  useAbilityTwoTargets (action) {
    ...
    if (abilityActionName1 == "RightClick" && abilityActionName2 == "HarvestLumber") {
      ...
      const selectionUnits = this.getSelectionUnits();
      const workers = selectionUnits.filter(u => u.meta.worker);
      const ghouls = selectionUnits.filter(u => u.itemId === 'ugho');

      // move regular workers from gold to lumber
      workers.forEach(() => {
        if (this.workerState.onGold > 0) {
          this.workerState.onGold--;
          this.workerState.onLumber++;
      ...
      // track ghouls sent to lumber (undead)
      ghouls.forEach(() => {
        this.ghoulState.onLumber++;
      });
