...
const EXPANSION_BUILDING_IDS = {
  ...
  'U': new Set(['ugol'])    // Haunted Gold Mine (starting one is pre-placed on map)
};
  ...
  addPlayerBuilding (unit, ignoreEvent = false) {
    ...
    if (raceExpIds && raceExpIds.has(unit.itemId)) {
      ...
      if (this.startingPosition) {
        ...
        // Fallback (no map position data): UD any ugol = expansion; others second+ = expansion
        isExpansion = (this.race === 'U') ? true : (prevCount >= 1);
      }
    }
