...
// Building pathing footprint in WPM cells (4 cells per tile).
// NE town halls have collisionSize=144 like production buildings, but are actually 4x4.
const BUILDING_WPM_FOOTPRINT = {
  'htow': 14, 'hkee': 14, 'hcas': 14,
  'ogre': 14, 'ostr': 14, 'ofrt': 14,
  'etol': 14, 'etoa': 14, 'etoe': 14,
  'unpl': 14, 'unp1': 14, 'unp2': 14,
};

function getBuildingWpmFootprint (itemId, collisionSize) {
  if (itemId && BUILDING_WPM_FOOTPRINT[itemId]) return BUILDING_WPM_FOOTPRINT[itemId];
  if (!collisionSize) return 8;
  if (collisionSize <= 130) return 8;
  if (collisionSize <= 160) return 12;
  return 14;
}
...
const Player = class {
  ...
  _applyTeleport (ability, caster, destX, destY, destBuilding) {
    ...
    // Enumerate grabbed units. Excludes: the caster, summons, illusions,
    // buildings, other heroes (TP only grabs non-hero allies), destroyed.
    const grabbed = [];
  ...
  addPlayerBuilding (unit, ignoreEvent = false) {
    ...
    // block the building's footprint on the pathfinding grid
    // (skip initial buildings — their footprints are already in the WPM pathing data)
    if (unit.currentX && unit.currentY && this.world.pathFinder) {
      const cs = (unit.balanceInfo && unit.balanceInfo.collisionSize) || 0;
      const footprint = getBuildingWpmFootprint(unit.itemId, cs);
      this.world.pathFinder.blockArea(unit.currentX, unit.currentY, footprint);
    }
  ...
  _findBuildablePosition (wpmGrid, building) {
    ...
    const cs = (building.balanceInfo && building.balanceInfo.collisionSize) || 72;

    // footprint in WPM cells (32 units each)
    const footprint = getBuildingWpmFootprint(building.itemId, cs);
    ...
    this.units.filter(u => u.isBuilding && (u.currentX !== 0 || u.currentY !== 0)).forEach(b => {
      ...
      const bCS = (b.balanceInfo && b.balanceInfo.collisionSize) || 72;
      const bFoot = getBuildingWpmFootprint(b.itemId, bCS);
  ...
  checkForSelectSubgroupEarlyExit (action) {
    ...
    if (Building.isNeutralHireBuilding(fixedItemId)) {
  ...
  useAbilityWithTargetAndObjectId (action) {
    ...
    switch (abilityActionName) {
      ...
      // 'HeroItem1' wins. So all targeted item uses funnel here and we
      // identify the actual item by inspecting the hero's inventory slot.
      // Today only TP Scroll needs special handling; other targeted items
      // (Healing Salve, Clarity Potion, Dust of Appearance) already fire
      // through the action-0x10 HeroItemN path in Hero.doAbilityNoTargetItemArray.
      case 'HeroItem1':
      case 'HeroItem2':
      case 'HeroItem3':
      case 'HeroItem4':
      ...
      case 'HeroItem6': {
        ...
        if (heroItem && heroItem.itemId === 'stwp') {
          this.addEvent('teleportScroll', { unit: firstUnit.exportUnitReference() });
          this.addEvent('itemUse', {
            item: { displayName: 'Scroll of Town Portal', itemId: 'stwp', knownItemId: 'stwp' },
            unit: firstUnit.exportUnitReference(),
            usesLeft: heroItem.expires ? (heroItem.usesLeft - 1) : null,
            category: 'consumable'
          });
          const ability = teleportAbilities && teleportAbilities['stwp'];
          if (ability && firstUnit) {
            ...
            this._applyTeleport(ability, firstUnit, tx, ty, targetBuilding);
          }
        }
        // Non-TP targeted items fall through silently (the no-target path in
        // Hero.doAbilityNoTargetItemArray already emitted itemUse for them).
      } break;
      case 'EatTree':
  ...
  selectGroundItem (action) {
    ...
    const itemId = action && action.itemId ? action.itemId : null;
    ...
    hits.forEach(hit => {
      ...
      if (group && typeof group.addItemEvent === 'function') {
        group.addItemEvent(this, actor, x, y, itemId);
      }
    });
  }
