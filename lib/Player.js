...
const Player = class {
  ...
  _isSameEvent (a, b) {
    ...
    switch (a.key) {
      case 'addBuilding': {
        ...
        if (ab.itemId && ab.itemId === bb.itemId) {
          ...
          // same position → same instance
          if (ab.currentX != null && ab.currentX === bb.currentX &&
              ab.currentY != null && ab.currentY === bb.currentY) return true;
        }
        return false;
      }
      case 'addUnit': {
  ...
  // and pass the _isSameEvent identity check.
  //
  deduplicateEventStream () {
    ...
    for (let i = 0; i < this.eventStream.length; i++) {
      ...
      for (let j = i + 1; j < this.eventStream.length; j++) {
        ...
        if (a.key !== b.key || a.gameTime !== b.gameTime) continue;
