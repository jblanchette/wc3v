...
const PathFinder = class {
  ...
  setup (gridData) {
    ...
    if (true || !fs.existsSync(gridPath)) {
      fs.writeFileSync(`${gridPath}`, astarGraph.toString());
      this.zipGameFile(gridPath);
    }
  ...
  findPath (startX, startY, endX, endY) {
    ...
    if (!worldPath.length && isDifferentSpot) {
      console.logger("ERROR - found broken path");
    }

    console.logger("path len: ", worldPath.length);
