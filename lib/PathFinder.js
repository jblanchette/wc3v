...
const PathFinder = class {
  ...
  setup (gridData) {
    ...
    const { xScale, yScale, cameraRatio, middleX, middleY } = this.gameScaler;
    ...
    // dev note:  i am bad at math.  for some reason this seems
    //            to 'magically' align the tree positions in the grid
    //            to not have a weird x offset issue.
    //            there is probably a better fix but this gets so close
    //            and looks great so shrug
    
    const magicCameraFix = {
      x: 1.0702,
      y: 1
    };
    ...
    dooGrid.forEach((item, index) => {
      ...
      const minX = (x * magicCameraFix.x);
      const minY = (y * magicCameraFix.y);

      const maxX = (minX + scaledSize);
      const maxY = (minY + scaledSize);
    ...
    for (let row = 0; row < pathGridWidth; row++) {
      ...
      for (let col = 0; col < pathGridHeight; col++) {
        const rowOffset = row + offsets.width;
        const colOffset = col + offsets.height;

        const gridPosition = this.getAdjustedGridPosition(
          rowOffset, colOffset, cameraBox, tileSize);
