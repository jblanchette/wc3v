...
const PathFinder = class {
  ...
  setup (gridData) {
    ...
    grid.forEach(item => {
      const scaledSize = treeSize * item.yScale;
    ...
    for (let row = 0; row <= pathGridWidth; row++) {
      ...
      for (let col = 0; col <= pathGridHeight; col++) {
        ...
        let spotWeight = tree.collides(gridHitBox) ? 0 : 1;
  ...
  findPath (startX, startY, endX, endY) {
    ...
    if (gridStart.row > maxGridRow || gridEnd.row > maxGridRow 
      ...
      console.log("error pathing to: ", startX, startY, endX, endY);
      console.log("gird start: ", gridStart, "grid end: ", gridEnd);

      console.log("grid rows: ", meta.length);
      console.log("grid cols: ", meta[0].length);

      console.log("max grid rows: ", maxGridRow);
      console.log("max grid cols: ", maxGridCol);
