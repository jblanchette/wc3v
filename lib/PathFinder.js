...
const PathFinder = class {
  ...
  setup (gridData) {
    ...
    dooGrid.forEach((item, index) => {
      ...
      const scaledSize = (treeSize / 2) * parseFloat(item.xScale);
  ...
  getCameraBox (bounds) {
    ...
    // detect if we need to flip the offset because of a left or top leaning center point

    if (Math.abs(cameraTiles.left) > Math.abs(cameraTiles.right)) {
      cameraBox.xTileOffset *= -1;
    }

    if (Math.abs(cameraTiles.top) > Math.abs(cameraTiles.bottom)) {
      cameraBox.yTileOffset *= -1;
    }
