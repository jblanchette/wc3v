...
const Unit = class {
  ...
  morphTo (itemId, gameTime) {
    ...
    const fromWorker = !!(this.meta && this.meta.worker);
    this.itemId = itemId;
    this.setUnitMeta();
    ...
    this.morphHistory.push({
      ...
      from: fromItemId,
      fromDisplayName,
      // Whether THIS form is a worker. The client has no unit-meta table for
      // forms a unit is not currently in, and it needs the answer per frame:
      // a peasant under Call to Arms is a militia, and a militia is not a
      // worker, so the viewer's worker declutter must not apply to it while it
      // is converted. Carrying the flag on the entry keeps that a pure lookup.
      worker: !!(this.meta && this.meta.worker),
