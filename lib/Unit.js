  ...
  moveTo (world, targetX, targetY, onCompleteExtra = null) {
    if (this.state === "walking") {
      ...
      if (this.moveInfo.timerEvent) {
        ...
        console.log("broken walk cancel: ", this.displayName, this.moveInfo);
      }
    }

    console.logger("unit move to: ", targetX, targetY);

    const walkPathTest = world.pathFinder.findPath(
      this.currentX, this.currentY, 
      targetX, targetY
    );

    console.logger(`walk path test:`);
    console.logger(`(${this.currentX}, ${this.currentY}) -> (${targetX}, ${targetY})`);

    console.logger("path len: ", walkPathTest.length);

    walkPathTest.forEach(testNode => {
      console.logger(testNode);
    });

    const walkPath = [{ x: targetX, y: targetY }];
    if (walkPath.length) {
