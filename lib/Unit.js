...
const Unit = class {
  ...
  clearMoveInfo () {
    if (this.itemId === "Udea") {
      console.logger("clearing move info here");
    }
  ...
  checkStateForMove () {
    ...
    if (this.moveInfo.timerEvent) {
      if (this.moveInfo.onCompleteCallback) {
        console.logger("unit has onCompleteCallback, calling it.");
        this.moveInfo.onCompleteCallback(false);
      } else {
        console.logger("unit had no onCompleteCallback: ", this.uuid, this.state);
      }

      this.eventTimer.cancelEvent(this.moveInfo.timerEvent);
    } else {
      console.logger("unit had no moveInfo: ", this.uuid);
    }
  }

  moveToBuild (world, building, onComplete) {
    console.logger("calling move to build: ", this.uuid);
    console.logger("state before build: ", this.state);
    this.moveTo(world, building.currentX, building.currentY, onComplete, MovementStates.BUILDING);
    console.logger("unit event uuid after call: ", this.moveInfo);
  }
  ...
  moveTo (world, targetX, targetY, onCompleteCallback = null, updateState = null) {
    ...
    console.logger("moving unit id: ", this.uuid, "state: ", this.state);
    console.logger("unit move to: ", targetX, targetY, "from: ", this.currentX, this.currentY);
    ...
    if (walkPath.length) {
      ...
      this.walkPath = [];
      console.logger("no walk path found for unit: ", this.uuid);

      if (onCompleteCallback) {
        console.logger("calling onCompleteCallback with no walk path: ", this.uuid);
