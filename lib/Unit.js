...
const Unit = class {
	constructor (eventTimer, itemId1, itemId2, knownItemId, isSpawnedAtStart = false, summonDuration = null ) {
		...
		console.logger("set unit spawned: ", this.isSpawnedAtStart, this.displayName);
	}
	...
	recordPosition () {
		...
		console.logger("recorded path for: ", this.displayName);
	}
	...
	moveToBuild (building, onComplete) {
		if (this.state === "walking") {
			...
			console.logger("stopping walking event before moveToBuild", this.state);
			console.logger("move info: ", this.moveInfo);
		...
		console.logger("calling moveTo from build");
	...
	moveTo (targetX, targetY, onCompleteExtra = null) {
		...
		console.logger("unit walk:", this.displayName);
		...
		if (this.state === "walking") {
			...
			console.logger("stopping walking event before moveTo");
			this.eventTimer.cancelEvent(this.moveInfo.timerEvent);
		...
		const ms = (this.meta.movespeed || 50);
		...
		if (isNaN(this.moveInfo.pathTime)) {
			console.logger(this.moveInfo);

			throw new Error("nan here");
		}

		if (pathTime > 3) {
			console.logger("*** unit is walking long path time: ", pathTime);
		}
		...
		if (this.isSpawnedAtStart && !this.path.length) {
			...
			console.logger("moving spawned unit to spot", targetX, targetY);

			this.currentX = targetX;
			this.currentY = targetY;

			console.logger("calling cancel event for spawn at start / no path");
			this.eventTimer.cancelEvent(this.moveInfo.timerEvent);
		}
	}
	...
	moveOnComplete (eventFinished) {
		...
		console.logger("move on complete. ", this.displayName, this.path.length);
		...
		if (onCompleteExtra) {
			console.logger("runnning on complete extra");
