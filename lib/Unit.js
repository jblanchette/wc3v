...
const Unit = class {
	...
	registerItemIds (itemId1, itemId2) {
		...
		if (utils.isEqualItemId(this.itemId1, badC)) {
			...
			//throw new Error("here bad c");
		}

		console.logger("unit reg itemIds:", itemId1, itemId2);
	...
	moveToBuild (building, onComplete) {
		if (this.state === "walking") {
			...
			console.logger("stopping walking for moveToBuild");
			this.eventTimer.cancelEvent(this.moveInfo.timerEvent);
		...
		console.logger("calling moveTo from moveToBuild");
	...
	moveTo (targetX, targetY, onCompleteExtra = null) {
		...
		if (this.state === "walking") {
			...
			console.logger("unit cancel walking:", this.displayName);
			this.eventTimer.cancelEvent(this.moveInfo.timerEvent);
			this.clearMoveInfo();
		}
		...
		if (this.moveInfo.onCompleteExtra) {
			console.logger("has onCompleteExtra");
		}
		...
		console.logger("unit uuid: ", this.moveInfo.timerEvent.uuid);
	...
	moveOnComplete (eventFinished) {
		...
		console.logger("move on complete: ", this.displayName);
