...
const Unit = class {
	...
	moveToBuild (building, onComplete) {
		if (this.state === "walking") {
			...
			if (this.moveInfo.onCompleteExtra) {
				console.logger("running walk cancel onCompleteExtra");
				this.moveInfo.onCompleteExtra(false);
			} else {
				console.logger("no on complete extra in walk cancel");
			}
	...
	moveOnComplete (eventFinished) {
		...
		console.logger(`${this.displayName} finished move at (${this.currentX}, ${this.currentY})`);

		this.recordPosition();
		this.recordMoveHistory();

		if (onCompleteExtra) {
			console.logger("runnning on complete onCompleteExtra");
