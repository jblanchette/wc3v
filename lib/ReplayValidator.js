  ...
  _checkSupplyBuildings (player, playerId) {
    ...
    if (supplyBuildingsInUnits > supplyBuildingEvents) {
      this.warnings.push({
        ...
        player: playerId,
        severity: 'medium',
        details: `Player has ${supplyBuildingsInUnits} ${supplyBuildingId} in units but only ${supplyBuildingEvents} addBuilding events (${supplyBuildingsInUnits - supplyBuildingEvents} missing)`
      });
    }

    if (player._supplyBumps && player._supplyBumps.length) {
      this.warnings.push({
        type: 'SUPPLY_SAFETY_NET_FIRED',
        player: playerId,
        severity: 'low',
        details: `Supply safety net fired ${player._supplyBumps.length} time(s) — indicates missed supply buildings`
      });
    }
  }
