...
class ReplayValidator {
  // opts.parseTruncated — the { lastActionGameTimeMs, replayLengthMs,
  // missingMs, reason } record wc3v.js attaches to the replay when the action
  // stream stopped before the end of the game. Replay-level, not per-player,
  // but it invalidates every player's data equally, so it is recorded against
  // each of them.
  constructor (players, opts = {}) {
    this.players = players;
    this.parseTruncated = opts.parseTruncated || null;
    this.warnings = [];
    this.errors = [];
    this.corrections = [];
  ...
  validate () {
    Object.keys(this.players).forEach(playerId => {
      ...
      this._checkParseTruncation(playerId);
    });
  ...
  //
  // A truncated parse is the one failure mode where nothing else in this file
  // can help: every other check reads the event stream, and a truncated stream
  // is internally consistent right up to the point it stops. Before this check
  // existed, 1057610942_Moon_Soin_AutumnLeaves20 lost the last 4:28 of a 17:26
  // game to a thrown action handler and still scored 0.998 / 0.990 with zero
  // warnings. Critical severity: build orders, APM, dominance and the winner
  // verdict are all wrong, and no consumer should trust the file.
  //
  _checkParseTruncation (playerId) {
    if (!this.parseTruncated) return;

    const { lastActionGameTimeMs, replayLengthMs, missingMs, reason } = this.parseTruncated;
    const fmt = (ms) => `${Math.floor(ms / 60000)}:${String(Math.floor((ms % 60000) / 1000)).padStart(2, '0')}`;

    this._record({
      type: 'PARSE_TRUNCATED',
      severity: 'critical',
      player: playerId,
      details: `Action stream ends at ${fmt(lastActionGameTimeMs)} of ${fmt(replayLengthMs)} ` +
        `(${fmt(missingMs)} missing). Every stat is computed over a partial game. Cause: ${reason}`,
      lastActionGameTimeMs,
      replayLengthMs,
      missingMs
    });
  }
