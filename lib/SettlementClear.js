...
// are serialized by NeutralGroup.exportGroup(). Depends only on CampCreditModel
// (itself dependency-free) so it stays cheap in the browser parser bundle.
//

// Shared with NeutralGroup and the tuning harness, so the clearing-work
// requirement used here can never drift from the one that set clearedTime.
const { requiredMsFor } = require("./CampCreditModel");
...
const SettlementClear = class {
  ...
  run () {
    ...
    Object.values(world.neutralGroups).forEach(camp => {
      ...
      // Earliest building that settles this camp — but a building only proves a
      // clear if the creeps were not demonstrably alive AFTER it went up. On
      // maps where a camp sits near a starting base, an opening building lands
      // within the footprint at ~0:20 and used to "prove" the camp was cleared
      // before the game began, dragging clearedTime to the first seconds of the
      // replay while a hero was provably still hitting those creeps minutes
      // later. Proof of life is a fact and the building test is an inference,
      // so the fact wins and such a building is rejected as a false positive.
      const proofOfLife = SettlementClear.proofOfLife(camp);
      const settler = buildings.find(bld => {
        if (proofOfLife != null && bld.time < proofOfLife) return false;
  ...
  //
  // Last moment a creep of this camp was PROVEN alive: a right-click only
  // resolves to a live neutral, so an in-zone click by a hero/combat unit is
  // ground truth. Mirrors CampCreditModel's CLAMP_TO_PROOF_OF_LIFE.
  //
  static proofOfLife (camp) {
    let t = null;
    (camp.perPlayerEvents || []).forEach(e => {
      if (!e || e.stage !== 'interact-creep') return;
      if (e.zone === 'out') return;
      if (!(e.hasHero || e.hasCombatUnit)) return;
      if (t == null || e.gameTime > t) t = e.gameTime;
    });
    return t;
  }
  ...
  // the building time itself. Static so the tuning harness
  // (tools/camp-clear-audit.js) applies the identical clamp when it re-runs
  // the model offline.
  //
  static clampToSettle (camp, settleTime, currentClearedTime) {
    // A building placed before the creeps were last seen alive proves nothing
    // about this camp — leave the estimate alone rather than drag it backwards.
    const proof = SettlementClear.proofOfLife(camp);
    if (proof != null && settleTime < proof) return currentClearedTime;
    ...
    // Never below proven creep life.
    if (proof != null && clearEstimate < proof) clearEstimate = proof;

    return (currentClearedTime != null)
      ? Math.min(currentClearedTime, clearEstimate)
      : clearEstimate;
  }
  ...
  _applySettle (camp, settler, mine) {
    ...
    camp.clearedTime = SettlementClear.clampToSettle(camp, settleTime, camp.clearedTime);
