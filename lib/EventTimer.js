...
const EventTimer = class {
	...
	addEvent (runLength, onTick, onComplete) {
		...
		console.logger("event start:",eventItem.uuid);
	...
	cancelEvent (eventItem) {
		...
		console.logger("cancel event:", eventItem.uuid);
	...
	process (newGameTime) {
		...
		this.events.forEach(item => {
			if (item.completed) {
				//console.logger("item alread completed:", item.uuid);
				return;
			}
			...
			if (gameTime >= item.endTime) {
				...
				console.logger("complete event:", item.uuid);
