 ...
 *
 * Measured motivation (Aug 2026): 52% of enemy
...
class AnchorCorrection {
  constructor (playerManager) {
    ...
    this.stats = {
      ...
      skippedLifetime: 0, skippedPathEnd: 0, skippedNoUnit: 0, skippedFar: 0,
  ...
  _correctUnit (unit, player, anchors) {
    ...
    for (const a of anchors) {
      ...
      // A correction bigger than JUMP_DIST cannot be walked, so it used to be
      // emitted as isJump and the client rendered a hard snap. That trades an
      // invisible average-accuracy gain for a very visible artifact: a unit
      // teleporting across the map, which reads as a bug to anyone watching.
      // The measured accuracy win (holdout p90 -36%) came from the many SMALL
      // corrections; the handful of huge ones are also the least trustworthy
      // (a >1500wu disagreement is as likely a misattributed anchor as a
      // genuinely lost unit). So skip them: keep every correction the unit
      // could plausibly have walked, and leave the path continuous.
      //
      // Measured on 1129305842_Leon_Lucifer_AutumnLeaves20: 15 visible
      // teleports -> 2, and the 2 that remain are real in-game teleports
      // (Town Portal), which SHOULD snap.
      if (shift > JUMP_DIST) {
        if (!config.anchorSnapFar) { this.stats.skippedFar++; continue; }
        this.stats.jumps++;
      }
      ...
      if (shift > JUMP_DIST) sample.isJump = true;
