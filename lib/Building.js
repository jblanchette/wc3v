    ...
    switch (abilityFlags) {
      case abilityFlagNames.CancelTrainOrResearch:
        ...
        if (!focusUnit.trainedUnits.length) {
          ...
          // unblock the building's footprint on the pathfinding grid
          if (removeBuilding.currentX && removeBuilding.currentY && player.world.pathFinder) {
            const cs = (removeBuilding.balanceInfo && removeBuilding.balanceInfo.collisionSize) || 0;
            const footprint = cs <= 130 ? 8 : (cs <= 160 ? 12 : 14);
            player.world.pathFinder.unblockArea(removeBuilding.currentX, removeBuilding.currentY, footprint);
          }
      ...
      case abilityFlagNames.TrainUnit:
        ...
        if (researchMeta[itemId]) {
          ...
          if (unitInfo.isBuilding) {
            ...
            console.logger("New spawn unit position: ", focusUnit.currentX, focusUnit.currentY);
            newUnit.setSpawnPosition(focusUnit.currentX, focusUnit.currentY);
