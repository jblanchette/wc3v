...
const Building = class extends Unit {
  ...
  upgradeBuilding (newItemId, player) {
    ...
    const onBuildingComplete = (eventFinished) => {
      ...
      const tierPos = buildTiers.indexOf(newItemId);
      console.logger("*** JDEBUG: checking tier upgrade: ", tierPos, newItemId, player.tierStream);
      if (tierPos > -1) {
        player.upgradeTier();
      }
    };

    console.logger("** jdebug setting building: ", newItemId);
  ...
  static doAbilityNoTargetItemId (
    ...
    switch (abilityFlags) {
      ...
      case abilityFlagNames.TrainUnit:
        ...
        if (unitInfo && (unitInfo.isUnit || unitInfo.isBuilding)) {
