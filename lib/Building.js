...
const Building = class extends Unit {
	...
	upgradeBuilding (newItemId) {
		...
		const onBuildingComplete = (eventFinished) => {
			console.logger("building upgrade complete, setting itemId: ", newItemId);
		...
		console.logger("starting building upgrade:", newItemId, "build time: ", buildTime);
	...
	initItemStock (itemId) {
		...
		console.logger("giving shop initial stock of: ", itemId);
	...
	addStockReplenish (itemId) {
		...
		console.logger("added shop item stock replenish: ", itemId, "stock:", this.itemStock[itemId]);
	}
	...
	static doAbilityNoTargetItemArray (
		...
		console.logger("Building ability no target.");
		...
		switch (abilityFlags) {
			case abilityFlagNames.CancelTrainOrResearch:
				...
				if (!removeItem) {
					...
					console.logger("Building: ", firstUnit);
					return;
				}
		...
		switch (abilityActionName) {
			case 'NEUpRoot':
				console.logger("building uprooted.");
			...
			break;
			default:
				console.logger("unable to find ability action name for building ability");
			break;
		}
	...
	static doAbilityNoTargetItemId (
		...
		switch (abilityFlags) {
			...
			case abilityFlagNames.LearnSkillOrTrain:
				...
				console.logger("setting unit spawn pos:", focusUnit.currentX, focusUnit.currentY);
			...
			case abilityFlagNames.TrainUnit:
				if (unitInfo && unitInfo.isUnit) {
					if (unitInfo.isBuilding) {
						...
						console.logger(player.id, "Making trained unit: ", newUnit.displayName);		
						player.addPlayerUnit(newUnit);
						player.unregisteredUnitCount++;
					}
				}
			...
			case abilityFlagNames.CancelTrainOrResearch:
				if (unitInfo.isItem) {
					...
					if (shopUnit) {
						// why is focusunit not buyStockItem...
						focusUnit.printUnit();
						const boughtItem = focusUnit.buyStockItem(itemId);
						...
						console.logger("ERROR: could not find shop unit to give item to.");
					}
				} else {
					focusUnit.upgradeBuilding(itemId);	
					console.logger(
