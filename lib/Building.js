...
// Spawn-position helper: prefer ground rally, fall back to building center.
// CollisionWorld.resolvePosition() (server) then snaps to nearest free spot
// so units never start inside building footprints or other units.
function pickSpawnPositionForBuilding (building) {
  const rally = building.rallyPoint;
  if (rally && rally.type === 'ground' && rally.pt) {
    return { x: rally.pt.x, y: rally.pt.y };
  }
  // "unit" rally (e.g. clicked on a goldmine) leaves spawn at building —
  // the worker will path to the rally target via _pendingRole logic.
...
const Building = class extends Unit {
  ...
  exportUnit () {
    ...
    if (!entry) {
      ...
      if (!entry) {
        throw new Error('Building.exportUnit: no pathing manifest entry for ' + this.itemId);
      }
    }
