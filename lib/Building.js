				...
				if (unitInfo.isItem) {
					...
					if (rallyPoint && rallyPoint.type === "unit") {							
						...
						if (shopUnit) {
							...
							console.logger(player.id, "Shop has known unit rally, giving item: ", shopUnit.displayName );
						} else {
							console.logger("ERROR: could not find shop unit to give item to.", rallyPoint);
						}
					} else {
						...
						console.logger(player.id, `Giving item ${unitInfo.displayName} to ${closestHero.displayName}`);
