...
const Building = class extends Unit {
	...
	upgradeBuilding (newItemId) {
		...
		const onBuildingTick = (gameTime, delta)  => {
			//console.log("building tick.", self.displayName);
		};
	...
	static doAbilityNoTargetItemArray (
		...
		switch (abilityFlags) {
			case abilityFlagNames.CancelTrainOrResearch:
				...
				console.log("WARNING: should remove some non-finished unit: ", removeUnit.displayName);
	...
	static doAbilityNoTargetItemId (
		...
		switch (abilityFlags) {
			...
			case abilityFlagNames.LearnSkillOrTrain:
				...
				console.log(player.id, "Making trained unit:", newUnit.displayName, itemId);	
				...
				if (unitInfo.isItem) {
					...
					if (rallyPoint && rallyPoint.type === "unit") {							
						...
						if (shopUnit) {
							...
							console.log("ERROR: could not find shop unit to give item to.", rallyPoint);
						}
					} else {
