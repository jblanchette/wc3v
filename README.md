...
![Three new ways to read a replay — skill bands, guided walkthroughs, head-to-head compare](marketing/launch-rollup.png)

Three new entry points into the same library, depending on where you are in your WC3 journey — pick a [skill band](#three-skill-bands) in the top nav, hit [Guide me](#guided-walkthrough) on any replay for a 12-step coached run-through, or drop your own `.w3g` to see a [milestone scoreboard](#beginner-mode-comparison-scoreboard) against the closest pro game.
...
![Three skill bands — New to WC3, Ladder, Pro meta](marketing/social-bands.png)

A site-wide switch in the top nav ([client/js/BandSwitcher.js](client/js/BandSwitcher.js), persisted to `localStorage`) picks the audience the site renders for. Same library, three framings:

- **New to WC3** — _Your first builds — forgiving timings and the few things that actually matter._ Simpler homepage cards, beginner-friendly build order, guided walkthrough surfaced on every replay.
...
![Guided walkthrough — a 12-step coached run-through with camera framing and why-it-matters lines](marketing/social-walkthrough.png)

`▸ Guide me` on any replay opens a 12-step coached run-through. Each step jumps the scrubber, frames the camera (zoom to base / follow the hero / fit both armies / walk the creep route), highlights the matching build-order rows, and shows a short coaching line — `action` (what happened) · `contrast` (what the opponent did) · `why` it matters · `takeaway` for your own games.

Step roster: **the opening** (top-to-bottom command order with worker / building / hero rows) · **the route to level 3** (creep camps cleared, in order) · **the level 3 spike** (skill picks for both heroes, side-by-side, with L1→L2 stat deltas for the doubled-up basic) · **Tier 2/3 timing** · **expansion** · **first army upgrade** · **counters** (e.g., your first Destroyers when they have 3+ casters out) · **siege** · **midgame composition check** · **final composition**.

Voice is grounded and factual — short "why it matters" lines from a small knowledge base, the rest is the actual game. Generated by [client/js/ReplayGuide.js](client/js/ReplayGuide.js); rendered by the walkthrough HUD in [client/js/app.js](client/js/app.js).

### Beginner-mode comparison scoreboard
...
- **Teching up** — T2 / T3 timing, second base.
- **Army built** — icon strips of every unit type with `×N` count badges; hover for name, attack type, armor type. Stacked per-player so a long roster reflows naturally instead of wrapping into 80-px columns.

No advice in this panel — just milestones a new player can read to gauge how fast they're going.
