...
## Race-specific worker mechanics

The four races build differently, and a faithful simulation has to model that. WC3V uses a `BuildMechanic` enum (in `helpers/mappings.js`) and a `raceBuildMechanic(race, buildingItemId)` lookup to pick the right behavior per race per building.

| Race | Mechanic | Description |
|---|---|---|
| Orc | `CONSUMED_TEMPORARY` | Peon enters the building, gets released when construction completes |
| Undead | `SUMMONER` | Acolyte summons the building, immediately frees up |
| Night Elf (ancient) | `CONSUMED_PERMANENT` | Wisp is sacrificed; food supply decrements |
| Night Elf (non-ancient) | `CONSUMED_TEMPORARY` | Wisp consumed during build, released on completion |
| Human | `BUILDER` | Peasant works on-site, can be pulled, multiple peasants supported |

`Building.startConstruction(buildTime, onComplete)` uses `EventTimer` for completion tracking (no setTimeout/setInterval — all timed events go through the simulation clock). Worker counts on the player's `eventStream` distinguish gold / lumber / on-build / consumed / training so the client can render race-correct economy charts.

A note on Undead lumber: ghouls harvest lumber, not acolytes. The server tracks this in `workers.ghoulsOnLumber` separately from `workers.onLumber` — clients summing lumber must add both fields.

## Research and upgrade tracking

90 upgrades are tracked across all races (attack, defense, ability) with per-level costs and timing. The metadata lives in `helpers/researchMeta.json`, generated by `tools/parse-upgrade-data.js` from raw WC3 game data (`upgradedata.slk` + `{race}upgradefunc.txt`).

The fix that made this work was discovering that research actions in W3C/FLO replays arrive via the `TrainUnit` ability flag (0x46), not `CancelTrainOrResearch` — items that are neither a unit nor a building. Before WC3V looked them up in `researchMeta`, those events were silently dropped. `Building.doAbilityNoTargetItemId()` and `Building.upgradeBuilding()` now check `researchMeta[itemId]` first, then fall through to unit/building dispatch.

Each research event lands in both the player's `eventStream` (key='research') and a dedicated `researchStream` for clients that just want the upgrade timeline without filtering events.

## Expansion detection

A town hall is flagged as an expansion (`isExpansion`) when it's placed more than 2500 units from the player's known starting position. Implemented in `Player.addPlayerBuilding()` with `EXPANSION_BUILDING_IDS` per race (H→`htow`, O→`ogre`, E→`etol`, U→`ugol`).

The client emits these as `expansion` events instead of regular building events, rendering a dedicated gold-toned banner in the build order panel. Verified: Happy's `etol` expansion at 3:52 on Concealed Hill correctly flagged; same map without expansion produces no false positives.

## Replay validation

`lib/ReplayValidator.js` runs after parsing, before output. It checks for:

- **`TIER_BUILDING_MISMATCH`** — building requires a tier the player isn't shown to have
- **`TIER_UNIT_MISMATCH`** — unit produced before its required tier upgrade event
- **`MISSING_WORKER_DATA`** — worker stream missing for non-neutral player

Validation results are written to the `.wc3v` output as a `validation` key only if warnings or errors are present. Neutral players (id ≥ 24) are skipped. The `correctTierMismatches()` method exists for auto-correction but is not yet wired into the pipeline.

The single biggest source of `TIER_*` mismatches was W3Champions / FLO replays, which don't emit action `0x10` (`UnitBuildingAbilityActionNoParams`). Tier upgrades, training commands, and research arrive via different paths in those formats. The fix is in `Player.selectSubgroup()` — when a building's `itemId` silently updates to match a known tier building (`tierBuildings` lookup), we trigger `upgradeTier()`. Guarded by `this.tier < tierPos + 2` to avoid double-firing in non-W3C replays where both action `0x10` and the selection path exist.

## Map rendering pipeline

Each map (~80 of them) goes through a multi-stage pipeline before it can render in the browser:

1. **Extract** — `tools/data-tool.js` reads `.w3x` map files and dumps `war3map.wpm` (pathing), `war3mapUnits.doo` (doodads + neutral buildings), `war3map.w3e` (terrain) into `mapdata/{Map}/`.
2. **Render** — terrain is rasterized into `client/maps/{Map}/map.jpg` and `gridmap.jpg`. Tileset-aware ground colors come from `PALETTE_COLORS` (~100 4-char palette codes → hex) and `TILESET_EXTRAS` (per-tileset water/tree/cliff colors for 18 tilesets). Per-tile brightness noise via seeded `tileHash()` adds natural variation (±12% ground, ±15% cliff).
3. **Neutral buildings** — gold mines, shops, fountains extracted from `war3mapUnits.doo` are written to `client/maps/{Map}/neutralBuildings.json.gz`. `getUnitInfo()` flags `isGoldmine`, `isFountain`, `isInteractiveShop`. The client preloads icons and overlays them on the minimap; gold mine diamonds are also baked into the static `map.jpg`.
4. **Spawn camp filtering** — many maps have neutral creep groups within 1500 units of player starting positions for slots that aren't occupied (Lost Temple has 4 spawns but most matches are 2 players). `helpers/utils.js` `writeOutput()` removes those after `assignCampOrder()` — confirmed Lost Temple drops 25→23 groups while Concealed Hill is unaffected.

`tools/regen-maps.js` regenerates map images from already-extracted mapdata without needing the `.w3x` again. Useful when palette/tileset code changes.

## 3D terrain rendering

Three.js v0.160 powers a heightmapped, lit terrain mesh underneath the units. Coordinates: Three.js X → WC3 X; Three.js Z → -WC3 Y (top-down ortho camera). Canvas overlays remain 2D for units, buildings, and HUD elements layered on top of the 3D scene.

Supporting tools:

- `tools/convert-terrain-dds.js` — converts WC3 DDS terrain textures to web-compatible formats
- `tools/convert-mdx-to-gltf.js` — converts WC3 building MDX models to glTF for Three.js
- `tools/convert-building-textures.js`, `convert-splat-textures.js`, `convert-doodad-textures.js` — texture pipeline

`BroadcastCamera` orchestrates auto-camera modes (action focus on hero clusters, building focus on construction events) by driving D3 zoom. `FogOfWar` uses a custom shader to mask non-playable map edges. `PathTrailRenderer3D` draws hero movement trails directly in the 3D scene.

The 3D layer falls back gracefully — if textures/models aren't extracted, terrain renders flat-shaded against the tileset palette colors.

## In-browser parsing

The same Node.js parser code runs in the browser. `tools/build-parser-bundle.js` browserifies it (with shims for `fs`, `path`, `Buffer`, `zlib` from `node_modules` like `browserify-zlib`, `path-browserify`, etc.) into `client/js/vendor/wc3v-parser.bundle.js` — a single IIFE exposing `Wc3vParser.parseToWc3v()`.

Parsing happens off the main thread when the Web Worker stub (`client/js/parser-worker.js`) is available; otherwise it falls back to inline parsing on the main thread. Either way the flow is:

```
.w3g (ArrayBuffer)
   │
   ▼  UploadManager validates extension + magic
   │
   ▼  Wc3vParser.parseToWc3v() — same engine as Node, accepts a Buffer
   │   (skipTerrainRead: true, mapDataCache pre-fetched from /maps/)
   │
   ▼
.wc3v JSON  ──►  IndexedDB (MyReplays store)
   │
   ▼
Viewer / Compare
```

The parser accepts an `onProgress({ phase, percent, ... })` callback throttled to ~10/sec so the UI can drive a progress bar without taking over the loop. Phases: `metadata`, `parsing`, `postprocess`, `done`.

User replays never leave the browser. The IndexedDB store has a quota-aware FIFO eviction strategy when the user piles up too many. A `?local=ID` query param on `/viewer.html` hydrates a replay from IndexedDB — if a friend shares that URL, they see a friendly "this replay is local to the device that uploaded it" overlay instead of an error.

## Comparison engine

When a user clicks "Compare to Pro" on one of their replays, this is the flow:

1. **`SummaryExtract`** pulls a lightweight summary from the user's parsed JSON: race, matchup, opener (first hero), key units, archetype tags, map, game length, end-game tier.
2. **`CompareMatcher`** scores all 322 pro replays in the manifest against that summary — same race, same matchup, same build archetype, comparable game length, ideally same map. Returns a primary auto-match plus a list of alternative pro chips.
3. **`ReplayAnalyzer`** scores both replays head-to-head across categories:
   - **Macro** — supply-block windows, idle production, worker count vs game time
   - **Production** — unit composition vs. matchup standard, training overlap
   - **Item Economy** — hero items bought/used, item value per minute
   - **Idle Resources** — floating gold/lumber, unspent supply
   Each category gets a 0–100 score and a letter grade (A+, A, B, C, D, F-).
4. **Findings** — itemized callouts with timestamps ("Hit supply max at 16:30 and stalled production for 30s — build farms before they fill"). Sorted into a "Top things to fix" list.
5. **Charts** — `CompareCharts` renders SVG comparisons: supply curves over time, worker counts, hero level progression. Inline in the modal.
6. **Tab drill-downs** — Build (event-by-event), Tech (research timing), Economy (charts), Heroes (skill build), Creeps (route + XP), Upgrades (upgrade timing).

Compatibility checks are surfaced explicitly: same race ✓, same matchup ✓, same map ✓, comparable length ✓. When something doesn't match (e.g. different race composition), the affected categories show "NOT GRADED — pro played a different race composition" instead of an apples-to-oranges score.

The user can override the auto-matched pro via `AdvancedComparePicker` — a modal that filters the pro library by race, matchup, opener, and tournament.
