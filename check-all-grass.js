...
async function main() {
  ...
  const fileName = '32x32.all-grass.w3m';
  
  try {
    FS.mkdir('/tmpmap4');
    try { FS.unmount('/tmpmap4'); } catch(e) {}
    FS.mount(FS.filesystems.NODEFS, { root: mapDir }, '/tmpmap4');
    
    const mpq = await MPQ.open(`/tmpmap4/${fileName}`, 'r');
    const file = mpq.openFile('war3map.w3e');
    const w3eData = file.read();
    file.close();
    
    const w3ePath = `/tmp/all-grass-check.w3e`;
