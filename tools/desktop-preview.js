 ...
 * that has never run the app. --reonboard shows it the way a release policy
 * would bring it back on a machine already set up; --must-update shows the
...
// One small real replay, so the header peeks (the "which player are you"
// cards and the newest-replays table under a folder row) have bytes to read.
// By default every read_replay answers with this one file, whatever path was
// asked for: the shape of the table is what is being looked at, not the
// games in it.
//
// --peek=<substring> embeds up to ten real replays whose file name contains
// the substring instead, one per path, so the picker sees a spread of games
// with one recurring player (`--peek=Happy` puts Happy in every game) and
// draws the big card the way a real install would.
const PEEK_DIR = path.join(ROOT, 'replays');
const peekFiles = (() => {
  if (typeof args.peek !== 'string') return ['3380195801_Happy_Life_Last-Refuge-15.w3g'];
  const hits = fs.readdirSync(PEEK_DIR)
    .filter(f => f.endsWith('.w3g') && f.toLowerCase().includes(args.peek.toLowerCase()))
    .map(f => ({ f, size: fs.statSync(path.join(PEEK_DIR, f)).size }))
    .filter(x => x.size > 30000)
    .sort((a, b) => a.size - b.size)
    .slice(0, 10)
    .map(x => x.f);
  if (!hits.length) { console.error(`--peek=${args.peek} matches no replay in replays/`); process.exit(1); }
  return hits;
})();
const PEEK_REPLAYS = peekFiles.map(f => fs.readFileSync(path.join(PEEK_DIR, f)).toString('base64'));
...
const PEEK_REPLAYS = ${JSON.stringify(PEEK_REPLAYS)};
// One replay per path, stable: the same path always reads the same file.
const PEEK_BY_PATH = new Map();
const peekFor = (p) => {
  if (!PEEK_BY_PATH.has(p)) PEEK_BY_PATH.set(p, PEEK_REPLAYS[PEEK_BY_PATH.size % PEEK_REPLAYS.length]);
  return PEEK_BY_PATH.get(p);
};
...
// --must-update fakes a manifest whose minimum is above this build;
// --reonboard fakes one whose onboard_from is above the version that set up.
var POLICY = ${JSON.stringify(args['must-update']
  ? { version: '9.9.9', minimum: '9.9.9', onboard_from: null }
  : args.reonboard
    ? { version: '9.9.9', minimum: null, onboard_from: '9.9.9' }
    : { version: '1.0.9', minimum: null, onboard_from: null })};
...
// app.js spawns its parser worker at ./js/parser-worker.js, relative to the
// page. This page sits beside dist rather than in it, so the URL is rebased
// the same way the script tags are. The worker's own importScripts is
// relative to the worker file and needs nothing.
const RealWorker = window.Worker;
window.Worker = function (url, opts) {
  return new RealWorker(String(url).replace('./', '../dist/'), opts);
};
  ...
  core: {
    invoke: async (cmd, args) => {
      switch (cmd) {
        ...
        case 'app_version': return ${JSON.stringify(args['must-update'] ? '1.0.9' : 'preview')};
        case 'release_policy': return POLICY;
        case 'setup_version': return SHOW_SETUP ? null : '1';
        case 'relaunch': console.log('preview: relaunch (no app here)'); return null;
        ...
        // One real replay stands in for every file (see PEEK_REPLAY), so the
        // header peeks have something to read. A fresh copy each time: the
        // buffer is transferred to the worker and cannot be reused.
        case 'read_replay': return b64(peekFor(String(args.path)));
