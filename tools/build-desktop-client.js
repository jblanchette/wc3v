...
// Every map's world bounds and folder name, as the site serves it at
// /data/map-folders.json. SummaryBuild reads this to stamp `mapInfo` onto a
// stored summary, which is what lets anything place a world coordinate — a
// creep camp, a starting position — on the map image.
//
// Minified on the way in: it is 116 KB pretty-printed and about a third of that
// as one line, and nobody reads it in the app.
const MAP_FOLDERS = path.join(ROOT, 'client', 'data', 'map-folders.json');
...
const main = () => {
  ...
  if (!fs.existsSync(MAP_FOLDERS)) {
    console.error(`Map bounds table missing: ${path.relative(ROOT, MAP_FOLDERS)}`);
    process.exit(1);
  }
  const dataDir = path.join(DIST, 'data');
  fs.mkdirSync(dataDir, { recursive: true });
  fs.writeFileSync(
    path.join(dataDir, 'map-folders.json'),
    JSON.stringify(JSON.parse(fs.readFileSync(MAP_FOLDERS, 'utf8')))
  );
  ...
  console.log(`bounds:    map-folders.json, ${(fs.statSync(path.join(dataDir, 'map-folders.json')).size / 1024).toFixed(0)} KB`);
