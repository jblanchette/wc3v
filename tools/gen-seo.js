...
function main () {
  ...
  // 2b. Site stats ──────────────────────────────────────────────────────────
  // The same corpus numbers llms.txt already advertises, as a file small
  // enough for the homepage to fetch. It exists because the alternative was
  // typing "192 replays" into index.html, where nothing would ever tell us it
  // had gone stale — summaries-index.json itself is 145KB, far too heavy to
  // pull down for one integer. --check gates it like every other output.
  emit('data/site-stats.json', JSON.stringify({
    generatedAt: new Date().toISOString(),
    note: 'Counts rendered on the site. Regenerated by tools/gen-seo.js; ' +
          'replays is the public parsed corpus (same number as ' +
          'summaries-index.json count).',
    // Deliberately no map count. The distinct-`map` tally over the corpus is
    // 14, but "Turtle Rock" and "Turtle Rock v1.6 dwsl" are the same map under
    // two names, so the number reads a map higher than the truth. Not worth
    // publishing until the map names are normalised.
    replays: replayCount,
